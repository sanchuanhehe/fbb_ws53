---
hide:
  - toc
---

# HiSpark插件

HiSpark Studio for VS Code 插件面向智能设备开发者提供一站式集成开发环境，支持代码编辑、编译、烧录和调试等全流程开发能力，支持 C/C++ 语言，覆盖 Windows x86_64、Linux x86/aarch64（含 WSL，要求 Ubuntu 20.04 及以上版本）以及 macOS Apple Silicon 架构。

## 快速导航

---

- [插件指南](HiSparkStudioforVSCodeUserGuide.md)

    插件的安装、工程创建与导入、编译、烧录、调试等完整使用说明。

---

- [GUI指南](GUIProjectCreationAndUsage.md)

    GUI 工程创建与 GUI 模拟器使用说明。仅 Brandy/3322 系列芯片支持，Linux/WSL 环境下不支持。

---

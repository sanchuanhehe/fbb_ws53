...
| DMA 模式 | 连续或大批量数据时评估使用 | 可降低逐字节中断带来的 CPU 负担，是否启用应通过目标负载实测 |
| 中断模式 | 间歇或较小数据量时使用 | 实际可承受速率取决于系统负载、缓冲区和回调处理时间，不使用固定波特率作为统一分界 |
| `CONFIG_UART_BUS_ID` | 0 | UART 总线编号 |
| `CONFIG_UART_TRANSFER_SIZE` | 4 | 每次接收和回显的数据长度 |

...
```mermaid
flowchart TD
    W["<div style='width: 220px;'>写入请求</div>"] --> C{"目标区域<br/>是否为全1?"}
    C -->|是| D["<div style='width: 220px;'>直接写入<br/>1→0 按位编程</div>"]
    C -->|否| E["<div style='width: 220px;'>先擦除<br/>全区域恢复为1</div>"]
    E --> D
    D --> V["<div style='width: 220px;'>回读验证</div>"]
```
...
| 配置项 | 案例设置 | 使用要求 |
| --- | --- | --- |
| 擦除起始地址 | `0x003F3000` | 按实际 Flash 擦除粒度对齐 |
| 擦除长度 | `0x1000`（4KB） | 满足器件对齐要求，且不超出可擦除区域 |
| 测试区域 | 客户预留区前 4KB | 确认无有效数据；不得覆盖固件、启动或参数区域 |
| 单次读写长度 | 32 字节 | 修改时需确认器件页编程限制和驱动能力 |
| 擦除等待 | 擦除接口返回后继续 | 检查返回值；擦除期间不要操作同一 Flash |

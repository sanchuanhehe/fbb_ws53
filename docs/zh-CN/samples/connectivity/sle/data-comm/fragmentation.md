...
Server 和 Client 使用相同的无符号累加算法：

```c
checksum += data[index];
```

测试数据是 0～255 的字节序列重复 4 次：

```text
(0 + 1 + ... + 255) × 4 = 32640 × 4 = 130560
```

累加和可以发现部分长度或内容错误，但不能提供密码学完整性，也存在不同数据得到相同结果的可能。它不能替代 CRC、消息认证码或应用层可靠传输协议。
...
- 数据长度、分片负载、总片数和传输 ID均为固定演示值，不是通用文件传输协议。
- 分片头没有偏移字段；Client 只支持从 0 开始的严格顺序追加。
- 丢失一片后，后续分片都会触发序号错误；只有再次收到 0 号片才会重置接收状态。
- 重复收到 0 号片会无条件清空当前重组进度，不区分它是新会话还是重复包。
- 没有乱序缓存、接收位图、超时、重传、完成确认或断点续传。
- Server 没有“正在传输”标志；多个合法触发可创建多个发送任务，使用相同 Transfer ID 的分片可能交错。
- Client 不检查 `reserved`，也不检查每片 `checksum` 是否一致；完成时只使用最后一片携带的 `checksum`。
- 简单累加和不是 CRC 或安全校验。
- 20 ms 只是固定发送节奏，不是根据协议栈流控动态计算的等待时间。
- Server 与 Client 属于同一个 Kconfig `choice`，同一份固件只能选择一个角色。
- WS53 源码 README 记录该案例已通过双 WS53 以及 WS53/WS63 两个方向的实板验证；本文的协议字段和代码行为仍以 WS53 C 源码为准。

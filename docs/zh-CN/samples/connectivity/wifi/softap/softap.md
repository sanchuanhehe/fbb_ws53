...
还应验证错误密码、多个客户端上下线、DHCP 地址分配与回收、地址池耗尽、SoftAP 重启、信道兼容性和长时间运行等场景。连接失败时依次检查终端是否能发现 SSID、信道是否受支持、安全类型与密码是否匹配；关联成功但没有 IP 时，重点检查 `ap0` 地址和 DHCP Server。

构建方法参见[快速入门](../../../../get-started/quick-start.md)。量产固件必须替换示例密码。若需要让 SoftAP 客户端访问 STA 上游网络，还必须实现路由、转发及必要的 NAT，参见[中继设计说明](../repeater.md)。

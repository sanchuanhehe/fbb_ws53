...
| 成员名称 | 数据类型 | 描述 |
| ------- | ------- | ---- |
| tx_freq | uint8_t | 发送频率，取值范围 0~0x4E，实际频率为 2402+x |
| tx_power | uint8_t | 发射功率，取值见 [sle_rf_power_t](#enum_sle_rf_power_t) |
| test_data_len | uint16_t | 发射测试数据长度，取值范围 0~0xFF |
...
| tx_interval | uint16_t | 射频发射间隔，取值范围 6~0xFFFF，1 个单位长度为 125us |
...
| 成员名称 | 数据类型 | 描述 |
| ------- | ------- | ---- |
| rx_freq | uint8_t | 接收频率，取值范围 0~0x4E |
...
| rx_interval | uint16_t | 射频接收间隔，取值范围 0~0xFFFF，1 个单位长度为 125us |

...
import get_started_cli as cli_contract
...
ELF_PATH = "output/ws53/acore/ws53_liteos_app/application.elf"
FWPKG_PATH = (
    "output/ws53/fwpkg/pack_all_core/ws53_liteos_app/"
    "ws53_liteos_app_all_in_one.fwpkg"
)

TARGET_JSON = PurePosixPath("src/build/config/target_config/ws53/ws53.json")
TARGET_CONFIG = PurePosixPath(
    "src/build/config/target_config/ws53/target_config.py"
)
DEFAULT_CONFIG = PurePosixPath(
    "src/build/config/target_config/ws53/menuconfig/acore/"
    "ws53_liteos_app.config"
)
...
FBB_CLI_URL = "https://gitcode.com/HiSpark/hs-fbb-cli"
...
def check_metadata(repo: Repository) -> str:
    ...
    for page in EXPECTED_PAGES:
        ...
        require(nested_scalar(front, "branch") == "master",
                f"{page} applies_to.branch must be master")
...
def check_target_mapping(repo: Repository) -> str:
    ...
    require(config.get("chipName") == "ws53", "ws53.json chipName must be ws53")
    require(config.get("board") == "ws53", "ws53.json board must be ws53")
    require(config.get("requires", {}).get("min_cli_version") == MIN_CLI_VERSION,
            f"ws53.json min_cli_version must be {MIN_CLI_VERSION}")
    require(
...
def check_executable_cli_contract(repo: Repository) -> str:
    ...
    try:
        cli_contract.validate_document_source(cli_source)
        rendered = cli_contract.render_document(cli_source)
    except (KeyError, ValueError) as error:
        raise CheckFailure(f"invalid generated CLI contract: {error}") from error

    require(cli_contract.CLI_REPOSITORY == FBB_CLI_URL,
            "executable contract uses a different FBB CLI repository")
    require(cli_contract.CLI_VERSION == "1.2.1",
...
def check_platform_sources(repo: Repository) -> str:
    windows_compiler = PurePosixPath(
        "src/tools/bin/compiler/riscv/cc_riscv32_musl_b010/"
        "cc_riscv32_musl_win/libexec/gcc/riscv32-linux-musl/7.3.0/cc1.exe"
    )
    linux_compiler = PurePosixPath(
        "src/tools/bin/compiler/riscv/cc_riscv32_musl_b010/"
        "cc_riscv32_musl/libexec/gcc/riscv32-linux-musl/7.3.0/cc1"
    )

...
/**
 * @if Eng
 * @brief  Config wifi sta send probe request max times after beacon miss.
 * @param  [in]  max_times Send probe request max times, range:[1-33].
 * @retval ERRCODE_SUCC        Execute successfully.
 * @retval Other               Execute failed.
 * @else
 * @brief  配置sta在beacon miss之后发送probe request的最大次数。
 * @param  [in]  max_times 发送probe request最大次数，取值[1-33]。
 * @retval ERRCODE_SUCC 成功。
 * @retval Other        失败。
 * @endif
 */

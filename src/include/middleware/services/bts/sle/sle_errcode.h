...
/**
 * @if Eng
 * @brief  SLE SAF error code base.
 * @else
 * @brief  SLE SAF 错误码起始。
 * @endif
 */
#define ERRCODE_SLE_SAF_BASE   0x80006200

/**
 * @if Eng
 * @brief  SLE SAF error code end.
 * @else
 * @brief  SLE SAF 错误码结束。
 * @endif
 */
#define ERRCODE_SLE_SAF_END   0x8000623F
...
/**
 * @if Eng
 * @brief  SLE SAF module error code.
 * @else
 * @brief  SLE SAF 模块错误码。
 * @endif
 */
typedef enum {
    ERRCODE_SAF_ASM_SUCCESS_WITH_DATA = ERRCODE_SLE_SAF_BASE + 0x01,     /*!< @if Eng operate success with data
                                                                              @else   操作成功，有数据返回
                                                                              @endif */
    ERRCODE_SAF_ASM_UNSUPPORTED_OPCODE = ERRCODE_SLE_SAF_BASE + 0x02,    /*!< @if Eng unsupported opcode
                                                                              @else   不支持或无效的操作码
                                                                              @endif */
    ERRCODE_SAF_ASM_FORMAT_ERR = ERRCODE_SLE_SAF_BASE + 0x03,            /*!< @if Eng format error
                                                                              @else   格式错误
                                                                              @endif */
    ERRCODE_SAF_ASM_INVALID_EP = ERRCODE_SLE_SAF_BASE + 0x04,            /*!< @if Eng invalid endpoint identifier
                                                                              @else   无效的访问点标识
                                                                              @endif */
    ERRCODE_SAF_ASM_UNSUPPORTED_CODEC = ERRCODE_SLE_SAF_BASE + 0x05,     /*!< @if Eng unsupported audio stream codec
                                                                              @else   不支持或无效的音频编解码器参数
                                                                              @endif */
    ERRCODE_SAF_ASM_INVALID_COMMUNICATION = ERRCODE_SLE_SAF_BASE + 0x06, /*!< @if Eng invalid communication mode
                                                                              @else   无效的通信方式
                                                                              @endif */
    ERRCODE_SAF_ASM_COMMUNICATION_REJECT = ERRCODE_SLE_SAF_BASE + 0x07,  /*!< @if Eng reject communication
                                                                              @else   拒绝该通信方式
                                                                              @endif */
    ERRCODE_SAF_ASM_UNSUPPORTED_CONTEXT = ERRCODE_SLE_SAF_BASE + 0x08,   /*!< @if Eng unsupported stream context
                                                                              @else   不支持或无效的音频流类型
                                                                              @endif */
    ERRCODE_SAF_ASM_INVALID_PORT = ERRCODE_SLE_SAF_BASE + 0x09,          /*!< @if Eng invalid port
                                                                              @else   无效的端口号
                                                                              @endif */
    ERRCODE_SAF_ASM_INSUFFICIENT_RESOURCE = ERRCODE_SLE_SAF_BASE + 0x0A, /*!< @if Eng insufficient resource
                                                                              @else   资源不足
                                                                              @endif */
    ERRCODE_SAF_ASM_UNSPECIFIED = ERRCODE_SLE_SAF_BASE + 0x0B,           /*!< @if Eng unspecified error
                                                                              @else   其他错误
                                                                              @endif */
    ERRCODE_SAF_ASM_ROLE_ERR = ERRCODE_SLE_SAF_BASE + 0x0C,              /*!< @if Eng role error
                                                                              @else   角色错误
                                                                              @endif */
} errcode_sle_saf_t;

...
#include "efuse.h"
#include "tcxo.h"
#if defined(CONFIG_SEC_UPGRADE_VERSION_VALIDATE) && defined(WS53_PRODUCT_NONE)
...
#define EFUSE0_BASE_ADDR             0x57024000
...
static efuse_stru g_efuse_cfg[EFUSE_IDX_MAX] = {
    ...
    { 8,    128, EFUSE_IDX_RW }, // die id
    { 1120, 32,  EFUSE_IDX_RW }, // mcu ver
    { 1152, 8,   EFUSE_IDX_RW }, // flashboot ver
    { 1160, 8,   EFUSE_IDX_RW }, // params ver
    { 848,  256, EFUSE_IDX_RW }, // hash root public key
    { 1104, 16,  EFUSE_IDX_RW }, // msid
    { 1168, 1,   EFUSE_IDX_RW }, // sec verify enable
    { 1584, 48,  EFUSE_IDX_RW }, // mac_addr_01
    { 1728, 48,  EFUSE_IDX_RW }, // mac_addr_02
    { 1776, 48,  EFUSE_IDX_RW }, // mac_addr_03
    { 1824, 48,  EFUSE_IDX_RW }, // mac_addr_last
    { 1872, 128, EFUSE_IDX_RW }, // customer rsvd
    { 848, 256, EFUSE_IDX_RW }, // customer rsvd
};
...
#define FIRST_MAC_ADDR_START_BIT 1728
#define SECOND_MAC_ADDR_START_BIT 1776
#define THIRD_MAC_ADDR_START_BIT 1824

#define FIRST_MAC_LOCK_BIT 840
#define SECOND_MAC_LOCK_BIT 841
#define THIRD_MAC_LOCK_BIT 842
...
uint32_t efuse_read_jtag_ssi(uint8_t *data, uint16_t data_len)
{
    ...
    if ((ssi_jtag & 0x07) != 0 && (ssi_jtag & 0x07) != 0x07) {
        return ERRCODE_FAIL;
    }
    *data = (ssi_jtag == 0) ? 0 : 1;
    return ERRCODE_SUCC;
...
#define MICLDO_EN_REG           0x5703022C
#define MICLDO_EN_DELAY_2US     2
#define MICLDO_EN_DELAY_60US    60

void pmu_efuse_micldo_en(bool enable)
{
    if (enable == true) {
        reg32_setbit(MICLDO_EN_REG, POS_0);
        uapi_tcxo_delay_us(MICLDO_EN_DELAY_60US);
        reg32_setbit(MICLDO_EN_REG, POS_1);
    } else {
        reg32_clrbit(MICLDO_EN_REG, POS_1);
        uapi_tcxo_delay_us(MICLDO_EN_DELAY_60US);
        reg32_clrbit(MICLDO_EN_REG, POS_0);
    }
    uapi_tcxo_delay_us(MICLDO_EN_DELAY_2US);
}
...
static errcode_t set_rollback_version(uint32_t partition_id, uint32_t efuse_id, uint32_t efuse_size)
{
    uint32_t read_version;
    uint32_t write_version;
    partition_information_t partition = {0};
    errcode_t ret = uapi_partition_get_info(partition_id, &partition);
    params_key_area_t *key_area =
        (params_key_area_t *)(uintptr_t)(partition.part_info.addr_info.addr + FLASH_HEAD_ADDR);
    if (key_area->mask_params_key_version_ext == 0) {
        boot_msg0("set_rollback_version mask 0");
        return ERRCODE_SUCC;
    }
    /* Get Efuse Version */
    ret = efuse_read_item(efuse_id, (uint8_t *)(&read_version), efuse_size);
    /* Set Efuse Version */
    read_version = read_version & key_area->mask_params_key_version_ext;
    write_version = key_area->params_key_version_ext & key_area->mask_params_key_version_ext;
    if (ret != ERRCODE_SUCC || read_version > write_version) {
        return ERRCODE_SEC_IMAGE_INFO_VERSION_INVALID;
    }
    if (read_version < write_version) {
        pmu_efuse_micldo_en(true);
        ret = uapi_efuse_write_buffer(g_efuse_cfg[efuse_id].id_start_bit / BIT_TO_BYTE, (uint8_t *)&write_version,
            efuse_size);
        pmu_efuse_micldo_en(false);
    }
    return ret;
}

errcode_t set_board_rollback_version(void)
{
    /* Set SSB Version */
    errcode_t ret = set_rollback_version(PARTITION_SSB, EFUSE_FLASHBOOT_VER_ID, EFUSE_FLASHBOOT_VERSION_SIZE);
    /* Set Flashboot Version */
    ret |= set_rollback_version(PARTITION_FLASH_BOOT_IMAGE, EFUSE_PARAMS_VER_ID, EFUSE_FLASHBOOT_VERSION_SIZE);
    /* Set APP Version */
    ret |= set_rollback_version(PARTITION_CCPU_IMAGE, EFUSE_MCU_VER_ID, EFUSE_APP_VERSION_SIZE);
    if (ret != ERRCODE_SUCC) {
        boot_msg1("set_board_rollback_version failed, ", ret);
    }
    return ret;
}
#endif
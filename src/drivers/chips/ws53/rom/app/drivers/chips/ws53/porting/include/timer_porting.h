...
/**
 * @if Eng
 * @brief  Use TCXO to compensate timer.
 * @param  [in]  diff TCXO cycle count
 * @retval uint64_t  Timer compensation cycle.
 * @else
 * @brief  使用TCXO对Timer进行补偿
 * @param  [in]  diff TCXO cycle 计数
 * @retval uint64_t  Timer cycle 补偿值。
 * @endif
 */

...
#define TEST_RTC 0
#define TEST_RTC_IRQ RTC_0_IRQN
...
/**
 * @if Eng
 * @brief  Register hal funcs objects into hal_rtc module.
 * @param  [in]  index Index of the hardware rtc. For detail, see @ref rtc_index_t.
 * @else
 * @brief  将hal funcs对象注册到hal_rtc模块中。
 * @param  [in]  index 硬件定时器索引值，参考 @ref rtc_index_t 。
 * @endif
 */
...
/**
 * @if Eng
 * @brief  Unregister hal funcs objects from hal_rtc module.
 * @param  [in]  index Index of the hardware rtc. For detail, see @ref rtc_index_t.
 * @else
 * @brief  从hal_rtc模块注销hal funcs对象。
 * @param  [in]  index 硬件定时器索引值，参考 @ref rtc_index_t 。
 * @endif
 */

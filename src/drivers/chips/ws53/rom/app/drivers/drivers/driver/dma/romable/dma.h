...
typedef struct dma_ch_user_memory_config {
     ...
     *  @endif */
    uint32_t dest;
    /** @if Eng  Transf number of transfer width.
     *  @else    传输数据量。
     *  @endif */
    uint16_t transfer_num;
    /** @if Eng  Transfer channel priority(Lowest: 0 and Highest: 3).
...
typedef struct dma_ch_user_peripheral_config {
     ...
     *  @endif */
    uint8_t trans_type;
    /** @if Eng  Transfer direction of memory-to-periph or periph-to-memory or periph-to-periph.
     *           - 0: memory to periph
     *           - 1: periph to memory
     *           - 2: periph to periph
     *  @else    内存与外设之间或外设到外设的传输方向。
     *           - 0: 内存到外设
     *           - 1: 外设到内存
     *           - 2: 外设到外设
     *  @endif */
    ...
    /** @if Eng  Source address increment.
     *           - 0: increment
     *           - 1: decrement
     *           - 2: no change
     *  @else    源端地址增量模式。
    ...
    /** @if Eng  Destination address increment.
     *           - 0: increment
     *           - 1: decrement
     *           - 2: no change
     *  @else    目的端地址增量模式。
...
/**
 * @if Eng
 * @brief  DMA channel transfer done/error callback function pointer type of user.

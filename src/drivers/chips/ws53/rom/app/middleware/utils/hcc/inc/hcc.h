...
td_bool hcc_sched_cond_check(hcc_handler *hcc, hcc_trans_queue *queue);
td_bool hcc_check_queue_cond(hcc_handler *hcc, td_u8 dir, td_u8 q_id);
int hcc_thread_wait_queue_cond(TD_CONST td_void *param);
td_bool hcc_check_header_vaild(TD_CONST hcc_handler *hcc, TD_CONST hcc_header *hdr);
hcc_service_type hcc_queue_id_2_service_type(hcc_handler *hcc, hcc_queue_dir dir, hcc_queue_type queue_id);
hcc_service_type hcc_fuzzy_trans_queue_2_service(hcc_handler *hcc, hcc_queue_type queue_id);
td_void hcc_rx_proc(hcc_handler *hcc, hcc_header *hcc_head, td_u8 *buf, td_u8 *user_param,
                    hcc_adapt_priv_rx_process rx_proc);
ext_errno hcc_sched_transfer(hcc_handler *hcc);
ext_errno hcc_adapt_tx_param_check(hcc_handler *hcc, TD_CONST td_u8 *buf, td_u16 len,
    TD_CONST hcc_transfer_param *param);
td_void hcc_header_init(hcc_header *head, td_u16 len, hcc_transfer_param *param);
td_void hcc_init_unc_buf(hcc_unc_struc *unc_buf, td_u8* buf, td_u32 len, hcc_transfer_param *param);
ext_errno hcc_tx_proc(hcc_handler *hcc, hcc_trans_queue *queue, hcc_transfer_param *param, td_u8 *buf, td_u16 buf_size);

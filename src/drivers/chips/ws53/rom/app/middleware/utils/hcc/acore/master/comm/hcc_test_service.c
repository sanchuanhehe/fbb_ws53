...
td_u32 hcc_test_rx_sdio_proc(hcc_queue_type queue_id, td_u8 sub_type, td_u8 *buf, td_u32 len, td_u8 *user_param)
{
    td_u8 bus_type = HCC_BUS_IPC;
    hcc_transfer_param agent_transfer_param = {0};
    agent_transfer_param.user_param = user_param;
    agent_transfer_param.service_type = HCC_ACTION_TYPE_TEST;
    agent_transfer_param.sub_type = sub_type;
    agent_transfer_param.queue_id = queue_id;
    agent_transfer_param.fc_flag = 0;
    hcc_tx_data(bus_type, buf, len, &agent_transfer_param);
    return 0;
}

td_u32 hcc_test_rx_ipc_proc(hcc_queue_type queue_id, td_u8 sub_type, td_u8 *buf, td_u32 len, td_u8 *user_param)
{
    td_u8 bus_type = HCC_BUS_SDIO;
    hcc_transfer_param agent_transfer_param = {0};
    agent_transfer_param.user_param = user_param;
    agent_transfer_param.service_type = HCC_ACTION_TYPE_TEST;
    agent_transfer_param.sub_type = sub_type;
...
static hcc_adapt_ops g_hcc_test_sdio_agent_adapt = {
    .free = hcc_adapt_test_free,
    .alloc = hcc_adapt_test_alloc,
#ifdef CONFIG_HCC_SUPPORT_FLOW_CONTRL
    .start_subq = hcc_test_start_subq,
    .stop_subq = hcc_test_stop_subq,
#endif
    .rx_proc = hcc_test_rx_sdio_proc,
};

static hcc_adapt_ops g_hcc_test_ipc_agent_adapt = {
    .free = hcc_adapt_test_free,
    .alloc = hcc_adapt_test_alloc,
#ifdef CONFIG_HCC_SUPPORT_FLOW_CONTRL
    .start_subq = hcc_test_start_subq,
    .stop_subq = hcc_test_stop_subq,
#endif
    .rx_proc = hcc_test_rx_ipc_proc,
};


td_void hcc_test_sdio_agaent_init(td_void)
{
    hcc_service_init(HCC_BUS_SDIO, HCC_ACTION_TYPE_TEST, &g_hcc_test_sdio_agent_adapt);
}

td_void hcc_test_ipc_agaent_init(td_void)
{
    hcc_service_init(HCC_BUS_IPC, HCC_ACTION_TYPE_TEST, &g_hcc_test_ipc_agent_adapt);
}
#endif
#endif

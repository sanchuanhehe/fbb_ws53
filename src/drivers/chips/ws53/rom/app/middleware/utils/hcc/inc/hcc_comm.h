...
#define hcc_printf_err_log(fmt, arg...) pf_log_alter(0, 0, LOG_LEVEL_ERROR, fmt, var_args_cnt(unused, ##arg), ##arg)
#define hcc_dfx_print(fmt, arg...)      \
    do {                                \
        print_str("[HCC] " fmt, ##arg); \
        hcc_printf_err_log(fmt, ##arg); \
    } while (0)
#else

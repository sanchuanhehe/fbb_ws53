...
hcc_dfx *hcc_get_dfx(hcc_handler *hcc);
...
ext_errno hcc_dfx_init(hcc_handler *hcc)
{
    ...
    hcc_dfx *tmp_hcc_dfx = (hcc_dfx *)osal_kmalloc(sizeof(hcc_dfx), OSAL_GFP_KERNEL);
    if (tmp_hcc_dfx == TD_NULL) {
        return EXT_ERR_MALLOC_FAILURE;
    }
    (td_void)memset_s(tmp_hcc_dfx, sizeof(hcc_dfx), 0, sizeof(hcc_dfx));
    size_t que_stat_len = sizeof(hcc_queue_stat) * hcc->que_max_cnt;
    tmp_hcc_dfx->queue_stat[HCC_DIR_TX] = (hcc_queue_stat *)osal_kmalloc(que_stat_len, OSAL_GFP_KERNEL);
    if (tmp_hcc_dfx->queue_stat[HCC_DIR_TX] == TD_NULL) {
        osal_kfree(tmp_hcc_dfx);
        return EXT_ERR_MALLOC_FAILURE;
    }
    (td_void)memset_s(tmp_hcc_dfx->queue_stat[HCC_DIR_TX], que_stat_len, 0, que_stat_len);
    tmp_hcc_dfx->queue_stat[HCC_DIR_RX] = (hcc_queue_stat *)osal_kmalloc(que_stat_len, OSAL_GFP_KERNEL);
    if (tmp_hcc_dfx->queue_stat[HCC_DIR_RX] == TD_NULL) {
        osal_kfree(tmp_hcc_dfx);
        osal_kfree(tmp_hcc_dfx->queue_stat[HCC_DIR_TX]);
        return EXT_ERR_MALLOC_FAILURE;
    }
    (td_void)memset_s(tmp_hcc_dfx->queue_stat[HCC_DIR_RX], que_stat_len, 0, que_stat_len);
#ifdef CONFIG_HCC_SUPPORT_DFX_SRV
    size_t srv_stat_len = sizeof(hcc_service_stat) * hcc->srv_max_cnt;
    tmp_hcc_dfx->service_stat = (hcc_service_stat *)osal_kmalloc(srv_stat_len, OSAL_GFP_KERNEL);
    ...
    (td_void)memset_s(tmp_hcc_dfx->service_stat, srv_stat_len, 0, srv_stat_len);
#endif

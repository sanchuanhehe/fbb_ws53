...
#ifndef IPC_SW_MAIL_H
#define IPC_SW_MAIL_H
...
/**
 * @defgroup drivers_driver_ipc_sw_mail IPC Software Mailbox
...
/**
 * @if Eng
 * @brief  IPC mailbox status.
 * @else
 * @brief  IPC邮箱状态。
 * @endif
 */
typedef enum ipc_mailbox_status {
    IPC_MAILBOX_STATUS_IDLE,
    IPC_MAILBOX_STATUS_BUSY
} ipc_mailbox_status_t;
 ...
 * @endif
 */
errcode_t ipc_sw_send_msg_sync(ipc_msg_info_t *msg_info);
 ...
 * @endif
 */
errcode_t ipc_sw_send_msg_async(ipc_msg_info_t *msg_info);
 ...
 * @endif
 */
errcode_t ipc_sw_receive_msg(uint8_t src_core, uint8_t channel);
 ...
 * @endif
 */
errcode_t ipc_sw_register_rx_handler(ipc_rx_handler_info_t *handler_info);

...
#if !defined(CONFIG_MASTER_KEY_SEL_SUPPORT)
#define DEOB_UPDATE_KEY_SEL_MRK1        0
#define DEOB_UPDATE_KEY_SEL_USK         1
#define DEOB_UPDATE_KEY_SEL_RUSK        2
#else
#define DEOB_UPDATE_KEY_SEL_MRK1        0
#define DEOB_UPDATE_KEY_SEL_MRK0        1
#define DEOB_UPDATE_KEY_SEL_RUSK        2
#define DEOB_UPDATE_KEY_SEL_USK         3
#endif
...
static crypto_table_item g_rkp_deob_table[] = {
    {
        .index = CRYPTO_KDF_OTP_KEY_MRK1, .value = DEOB_UPDATE_KEY_SEL_MRK1
    },
#if defined(CONFIG_MASTER_KEY_SEL_SUPPORT)
    {
        .index = CRYPTO_KDF_OTP_KEY_MRK0, .value = DEOB_UPDATE_KEY_SEL_MRK0
    },
#endif
    {
        .index = CRYPTO_KDF_OTP_KEY_RUSK, .value = DEOB_UPDATE_KEY_SEL_RUSK
    },
    {
        .index = CRYPTO_KDF_OTP_KEY_USK, .value = DEOB_UPDATE_KEY_SEL_USK
    },
};
...
td_s32 hal_rkp_deob_update(crypto_kdf_otp_key otp_key, crypto_kdf_update_alg alg)
{
    ...
    td_u32 deob_key_reg_val = 0;
    td_u8 deob_alg_reg_val = 0;
    rkp_deob_cfg deob_cfg = { 0 };

    ret = crypto_get_value_by_index(g_rkp_deob_table, crypto_array_size(g_rkp_deob_table),
        otp_key, &deob_key_reg_val);
    crypto_chk_return(ret != TD_SUCCESS, ret, "invalid otp_key\n");
...
static const crypto_table_item g_rkp_alg_sel_table[] = {
    {
...
#ifdef CONFIG_MASTER_KEY_SEL_SUPPORT
#define HISI_MRK       0
#define OEM_MRK        2
static crypto_table_item g_master_key_table[] = {
    {
        .index = CRYPTO_MRK0, .value = HISI_MRK
    },
    {
        .index = CRYPTO_MRK1, .value = OEM_MRK
    },
};
#endif
...
td_s32 hal_rkp_kdf_hard_calculation(const crypto_kdf_hard_calc_param *param)
{
    ...
    td_u32 master_key_sel_reg_val = 0;
...
#ifdef CONFIG_MASTER_KEY_SEL_SUPPORT
    /* get master_key type. */
    ret = crypto_get_value_by_index(g_master_key_table, crypto_array_size(g_master_key_table),
        param->master_key_sel, &master_key_sel_reg_val);
    crypto_chk_return(ret != TD_SUCCESS, ret, "get master key type failed\n");
#endif
    /* config oneway. */
    ...
    for (i = 0; i < param->salt_length / CRYPTO_WORD_WIDTH; i++) {
        salt_word = 0;
        crypto_u8_to_word(param->salt, salt_word, i);
    ...
    cfgval.bits.master_key_sel = master_key_sel_reg_val;
...
td_void hal_rkp_debug(td_void)
{
    ...
    crypto_print("RKP_CMD_CFG: master_ket_sel is 0x%x\r\n", cmd_cfg.bits.master_key_sel);

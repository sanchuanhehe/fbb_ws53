/*
 * Copyright (c) HiSilicon (Shanghai) Technologies Co., Ltd.. 2008-2022. All rights reserved.
...
/**
 *    ot_drv_pm_register    -    register a media device
...
static int pm_register_get_media_minor(pm_device *media)
{
    ...
    if (media->minor == HIMEDIA_DYNAMIC_MINOR) {
    ...
    pm_basedev *bdev, dev_t *dev, struct device **adev)
{
    ...
    *dev = MKDEV(HIMEDIA_DEVICE_MAJOR, media->minor);
...
int ot_drv_pm_register(pm_device *media)
{
    int ret;
    dev_t dev;
...
/**
 *    ot_drv_pm_un_register - unregister a media device
 *    @media: device to unregister
 *
 *    Unregister a media device that was previously
 *    successfully registered with ot_drv_pm_register(). Success
 *    is indicated by a zero return, a negative errno code
 *    indicates an error.
 */
int ot_drv_pm_un_register(pm_device *media)
{
    unsigned int i;
    ...
    // 1.2
    if (media->app_device) {
        device_destroy(g_media_class, MKDEV(HIMEDIA_DEVICE_MAJOR, media->minor));
...
int drv_pm_mod_init(void)
{
    ...
    // 3
    ret = -EIO;
    if (register_chrdev(HIMEDIA_DEVICE_MAJOR, "media_char_dev", &g_media_fops)) {
        goto err2;
    }
    return 0;
    // 4
...
void drv_pm_mod_exit(void)
{
    ...
    // 1
    unregister_chrdev(HIMEDIA_DEVICE_MAJOR, "media");
    // 2
    class_destroy(g_media_class);
    // 3
    media_bus_exit();
...
EXPORT_SYMBOL(ot_drv_pm_register);
EXPORT_SYMBOL(ot_drv_pm_un_register);

...
static int media_match(struct device *dev, struct device_driver *drv)
{
    ...
    return (strncmp(pdev->name, drv->name, HIMEDIA_DEVICE_NAME_MAX_LEN + 8) == 0); /* 8 used calc len(drv->name) */
}
...
static int media_pm_freeze(struct device *dev)
{
    ...
    if (drv->pm) {
        ...
        ret = media_legacy_suspend(dev, PMSG_FREEZE);
    }
...
static int media_pm_freeze_noirq(struct device *dev)
{
    ...
    if (drv->pm) {
        ...
        ret = media_legacy_suspend_late(dev, PMSG_FREEZE);
    }
...
static int media_pm_poweroff(struct device *dev)
{
    ...
    if (drv->pm) {
        ...
        ret = media_legacy_suspend(dev, PMSG_HIBERNATE);
    }
...
static int media_pm_poweroff_noirq(struct device *dev)
{
    ...
    if (drv->pm) {
        ...
        ret = media_legacy_suspend_late(dev, PMSG_HIBERNATE);
    }
...
#define HIMEDIA_PM_OPS_PTR (&g_media_dev_pm_ops)
...
{
    struct media_drvobj *pa = container_of(drv, struct media_drvobj, pdrv);
    kfree(pa);
    return;
}

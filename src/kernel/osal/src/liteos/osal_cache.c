...
void osal_dcache_region_wb(void *kvirt, unsigned long phys_addr, unsigned long size)
{
    ...
    ArchDCacheCleanByAddr(phys_addr, end_addr);
#else
...
#ifdef LOSCFG_ARCH_ARM_CORTEX_A
    unsigned long end_addr;
    end_addr = (unsigned long)(UINTPTR)addr + size;
    ArchDCacheInvByAddr((UINTPTR)addr, end_addr);
#else
...
void osal_dcache_region_clean(void *addr, unsigned int size)
{
    ...
    unsigned long end_addr;
    end_addr = (unsigned long)(UINTPTR)addr + size;
    ArchDCacheCleanByAddr((UINTPTR)addr, end_addr);
#else

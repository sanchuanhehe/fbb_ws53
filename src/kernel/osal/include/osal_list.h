 ...
 */
static INLINE__ void osal___list_add(struct osal_list_head *_new,
 ...
 */
static INLINE__ void osal_list_replace(struct osal_list_head *old,
...
static INLINE__ void osal_list_replace_init(struct osal_list_head *old,
    struct osal_list_head *_new)
{
 ...
 */
static INLINE__ void osal_list_move_tail(struct osal_list_head *list,
 ...
 */
static INLINE__ int osal_list_is_last(const struct osal_list_head *list,
...
{
    return (osal_list_empty(head) == 0) && (head->next == head->prev);
}

static INLINE__ void osal___list_cut_position(struct osal_list_head *list,
 ...
 *
 */
static INLINE__ void osal_list_cut_position(struct osal_list_head *list,
    struct osal_list_head *head, struct osal_list_head *entry)
{
    if (osal_list_empty(head) != 0) {
        return;
...
static INLINE__ void osal___list_splice(const struct osal_list_head *list,
    struct osal_list_head *prev,
    struct osal_list_head *next)
{
 ...
 */
static INLINE__ void osal_list_splice(const struct osal_list_head *list,
    struct osal_list_head *head)
{
    if (osal_list_empty(list) == 0) {
 ...
 * @head: the place to add it in the first list.
 */
static INLINE__ void osal_list_splice_tail(struct osal_list_head *list,
    struct osal_list_head *head)
{
    if (osal_list_empty(list) == 0) {
 ...
 */
static INLINE__ void osal_list_splice_init(struct osal_list_head *list,
    struct osal_list_head *head)
{
    if (osal_list_empty(list) == 0) {
        osal___list_splice(list, head, head->next);
 ...
 * The list at @list is reinitialised
 */
static INLINE__ void osal_list_splice_tail_init(struct osal_list_head *list,
    struct osal_list_head *head)
{
    if (osal_list_empty(list) == 0) {
        osal___list_splice(list, head->prev, head);
...
/* next must be != NULL */
static INLINE__ void osal_hlist_add_before(struct osal_hlist_node *n,
...
static INLINE__ void osal_hlist_add_after(struct osal_hlist_node *n,
    struct osal_hlist_node *next)
{
    next->next = n->next;
    n->next = next;
 ...
 */
static INLINE__ void osal_hlist_move_list(struct osal_hlist_head *old,

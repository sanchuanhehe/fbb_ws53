...
def get_group_owner(data_dict, save_name, target_name):
    ...
    with open(OUTPUT_DIR / f"{save_name}_library_summary.csv", 'w', encoding='utf_8_sig') as f:
        ...
        f.write(f"{','.join(header)}\n")
       # 各模块规格限制 PLAT WIFI BTC BTH UNKNOWN  STACK PKTRAM
        lim_all = {'ws63-liteos-app.map_ram': {'PLAT' : 35.50, 'WIFI':46.6, 'RADAR':49, 'BTC': 20, 'BTH': 10, 'UNKNOWN': 0.5, 'STACK':7,'PKTRAM':50},
            # ws53 C core ram
            'control_ws53.map_ram':{'PLAT' : 16.97, 'WIFI':21.740, 'BTC': 32, 'BTH': 10, 'UNKNOWN': 0.1, 'STACK':6,'HEAP':27},
            # ws53 C core flash
    ...
    with open(OUTPUT_DIR / f"{save_name}_file.csv", 'w', encoding='utf_8_sig') as f:
        ...
        f.write(f"{','.join(header)}\n")
        wifi_group_dict = defaultdict(int)  # 用于WIFI Group的角度统计 地址大小
    ...
    with open(OUTPUT_DIR / f"{save_name}_file_summary.csv", 'w', encoding='utf_8_sig') as f:
        ...
        f.write(f"{','.join(header)}\n")
        lim_all = {'control_ws53.map_flash':{'WIFI_FRW' : 7.8, 'WIFI_PROT':18.3, 'WIFI_APP': 2.0, 'WIFI_FE': 28.5, 'WIFI_ALG': 1.9,'WIFI_UNKNOWN': 0.1},
            # ws53 A core WIFI flash
            'application.map_flash':{'WIFI_FRW' : 32.5, 'WIFI_PROT':197, 'WIFI_APP': 320.07, 'WIFI_FE': 16.4, 'WIFI_ALG': 39,'WIFI_UNKNOWN': 3.329}}

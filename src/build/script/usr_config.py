...
def mconfig(command, chip, core, target, output, assignments = None, root=""):
    ...
    # Out-of-tree builds invoke us from the project dir, but Kconfig paths
    # (config.in, .config files) are resolved relative to the SDK root. Chdir
    # there for the duration of this call, then restore. try/finally is
    # mandatory: any uncaught Kconfig error would otherwise leak the chdir
    # to the rest of the build and poison every subsequent module.
    cwd_changed = bool(os.environ.get('FBB_BUILD_ROOT_PATH'))
    original_cwd = os.getcwd() if cwd_changed else None
    if cwd_changed:
        os.chdir(root)
    try:
...
'''
        autoheader_header = '''/* Copyright (c) HiSilicon (Shanghai) Technologies Co., Ltd. 2022-2022. All rights reserved. */
'''
        ...
        if command == 'savemenuconfig':
            ...
            # Apply a defconfig fragment on top of the current .config,
            # then write back. Used by `fbb build` to overlay feature
            # defaults. The fragment path is read from FBB_FEATURES_DEFCONFIG.
            frag = os.environ.get('FBB_FEATURES_DEFCONFIG')
            if not frag:
                sys.exit("error: mergeconfig requires FBB_FEATURES_DEFCONFIG env")
            kconf.load_config()
        ...
        if cwd_changed and original_cwd:

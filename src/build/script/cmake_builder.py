...
class CMakeBuilder(BuildEnvironment):
    ...
    def start(self, env, target_name, output_path, nhso=None, clean=None):
        ...
        # sw21 sign if need
        if env.get('chip') == 'sw21':
            bin_name = env.get('bin_name')
            sign_py_path = os.path.join(root_path, 'build', 'config', 
                'target_config', env.get('chip'), 'sign_cfg', 'change_sign_cfg2.py')
            cmd = "%s %s -bin_name %s -bin_path %s"%(self.python_path, sign_py_path, bin_name, output_path)
            if (exec_shell(cmd, None) != 0):

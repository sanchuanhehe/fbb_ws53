...
def get_table_content_for_func_name_jump(files, func_addrs, func_patch_addrs, version, bt_rom_file_in):
    table_content = []
    ...
    while index < func_num:
        ...
        table_content.append(func_patch_addr)
        index += 1
...
def get_patch_symbol(nm_file_in):
    patch_symbol = []
    try:
        with open(str(nm_file_in), 'r') as f:
            content = f.readline()
            while content:
                pattern = r'\w+\$patch\$.*_hard*'
                matches = re.findall(pattern, content)
                for match in matches:
                    # print(match.split('|')[0])
                    patch_symbol.append(match.split('|')[0])
                content = f.readline()
    except Exception as e:
        print('Error:', e)
        sys.exit(1)
...
def get_origin_fun(patch_symbol):
    origin_func_names = []
    for symbol in patch_symbol:
        origin_func_names.append(symbol.split('$')[0])
        # print(symbol.split('$')[0])
    return origin_func_names

def get_patch_fun(patch_symbol):
    patch_func_names = []
    for symbol in patch_symbol:
        patch_fun = symbol.split('$')[2].replace("_hard", "")
        patch_func_names.append(patch_fun)
        # print(patch_fun)
    return patch_func_names

def create_patch(patch_info, nm_file_in, rom_bin_file, auto_patch_cfg = False):
    ...
    if auto_patch_cfg == "True":
        patch_symbol = get_patch_symbol(nm_file_in)
        funs = get_origin_fun(patch_symbol)
        funs_patch = get_patch_fun(patch_symbol)
    else:
    ...
    remap_type = ""
    if 'Table_Remap_Type' in file_all:
        remap_type = file_all['Table_Remap_Type']
    if remap_type == 'function':
        table_contents = get_table_content_for_func_name_jump(file_all, func_addrs, func_patch_addrs, file_all['Device_Code_Version'], rom_bin_file)
    else:
        if reg_size == 4:
...
def output_bin_file(file_all, output_dir_in, ram_bin_file):
    if os.path.exists(os.path.join(output_dir_in, ram_bin_file)):
        shutil.copy(os.path.join(output_dir_in, ram_bin_file), os.path.join(output_dir_in, "unpatch.bin"))
...
if __name__ == "__main__":
    if (len(sys.argv) == 8) or (len(sys.argv) == 9):
        ...
        if (len(sys.argv) == 9):
            auto_patch_cfg = sys.argv[8]
            create_patch(patch_info, nm_file, rom_bin_file, auto_patch_cfg)
        else:

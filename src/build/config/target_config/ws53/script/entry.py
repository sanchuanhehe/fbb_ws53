...
def do_cmd(target_name: str, hook_name: str, env: Dict[str, Any])->bool:
    ...
    if hook_name == 'build_pre' and env.get('build_type', '') == 'GuardSDKSample':
        ...
        for k, v in env.items():
            print(f"{k}:{v}")
        print("==================env==================")
        print("ws53_control_app start build .....")
        errcode = exec_shell([sys.executable, 'build.py', '-c', 'ws53_control_app'], None, True)
        if errcode != 0:
            print("ws53_control_app build failed!")
            sys.exit(1)
        # 生成sdk
        build_sdk()
        # 切换当前工作目录
        cwd = os.getcwd()
        in_sdk_root = os.path.join(output_root,'package', 'ws53','sdk')
    ...
    if env.get('build_type', '') == 'custom' and hook_name == 'build_post':
        custom_build_py = os.path.join(root_path, 'application/samples/custom/ws53/custom_build.py')
...
def build_sdk():
    ...
    if errcode != 0:
        ...
        print(f"build target pack_ws53_sdk  success!")
...
def guard_sample(target_name, env):
    ...
    source_fwpkg_dir = os.path.join(in_sdk_output_root, env.get('chip'), 'fwpkg', 'pack_all_core', f'{build_target}')
    ...
    for cfg_path in env.get('menuconfigs', []):
        ...
        all_in_one_fwpkg_path = os.path.join(source_fwpkg_dir, f'{build_target}_all_in_one.fwpkg')
        if os.path.isfile(all_in_one_fwpkg_path):
            dest_fwpkg = os.path.join(dest_fwpkg_dir, f'{cfg_base_name}_all_in_one.fwpkg')

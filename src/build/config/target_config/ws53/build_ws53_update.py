...
class upg_base_info:
    def __init__(self):
        ...
        # IAMGE
        self.image_bin = os.path.join(self.output, "acore", "ws53_liteos_app", "ws53_liteos_app_sign.bin")
        # testsuite IAMGE
        self.plt_image_bin = os.path.join(self.output, "acore", "ws53_liteos_plt_only", "ws53_liteos_plt_only_sign.bin")
        # NV
        self.nv_bin = os.path.join(self.output, "acore", "nv_bin", "ws53_all_nv.bin")

        # 签名加密前原始镜像
        self.src_boot = os.path.join(self.output, "acore", "ws53-flashboot", "flashboot.bin")
        self.src_app = os.path.join(self.output, "acore", "ws53_liteos_app", "ws53_liteos_app.bin")
        self.src_test = os.path.join(self.output, "acore", "ws53_liteos_plt_only", "ws53_liteos_plt_only.bin")

        # 差分升级时，以下路径配置为上一个版本的镜像路径
        # FlashBoot
        self.flashboot_old_bin = os.path.join(self.output, "acore", "old_version", "flashboot_sign.bin")
        # IAMGE
        self.image_old_bin = os.path.join(self.output, "acore", "old_version", "ws53_liteos_app_sign.bin")
        # testsuite IAMGE
        self.plt_image_old_bin = os.path.join(self.output, "acore", "old_version", "ws53_liteos_plt_only_sign.bin")
        # NV
...
def get_new_image(input,info):
    image_list = []
    if 'app' in input:
        image_list.append("=".join([info.image_bin, "application"]))
    if 'plt_image' in input:
        image_list.append("=".join([info.plt_image_bin, "plt_image"]))
    if 'boot' in input:
...
def get_old_image(input,info):
    image_list = []
    if 'app' in input:
        image_list.append("=".join([info.image_old_bin, "application"]))
    if 'plt_image' in input:
        image_list.append("=".join([info.plt_image_old_bin, "plt_image"]))
    if 'boot' in input:

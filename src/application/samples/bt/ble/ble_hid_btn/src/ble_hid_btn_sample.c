...
#define CONFIG_BLE_HID_BTN_PIN 6
#endif
...
/* HH-D170 S1 follows the proven SLE sample mapping: MGPIO6 is pin_t 32. */
#if CONFIG_BLE_HID_BTN_PIN == 6
#define HID_BTN_RUNTIME_PIN S_MGPIO6
#define HID_BTN_PRESSED_LEVEL GPIO_LEVEL_HIGH
#define HID_BTN_PULL_MODE PIN_PULL_NONE
#else
#define HID_BTN_RUNTIME_PIN ((pin_t)CONFIG_BLE_HID_BTN_PIN)
#define HID_BTN_PRESSED_LEVEL GPIO_LEVEL_LOW
#define HID_BTN_PULL_MODE PIN_PULL_UP
#endif
...
{
    bool raw = (uapi_gpio_get_val(state->pin) == HID_BTN_PRESSED_LEVEL);
...
static int hid_btn_task(const char *arg)
{
    hid_button_state_t state = {
        .pin = HID_BTN_RUNTIME_PIN,
        .keycode = (uint8_t)CONFIG_BLE_HID_BTN_KEYCODE,
    };
    unused(arg);

    /* Keep this sequence aligned with the working SLE S1 button sample. */
    uapi_pin_set_mode(state.pin, HAL_PIO_FUNC_GPIO);
    uapi_pin_set_pull(state.pin, HID_BTN_PULL_MODE);
    uapi_gpio_set_dir(state.pin, GPIO_DIRECTION_INPUT);
    osal_printk("%s task started, gpio=%u pin_t=%u keycode=0x%02X active=%s\r\n", BLE_HID_SAMPLE_TAG,
                (uint32_t)CONFIG_BLE_HID_BTN_PIN, (uint32_t)state.pin, state.keycode,
                HID_BTN_PRESSED_LEVEL == GPIO_LEVEL_HIGH ? "high" : "low");

...
typedef struct {
    ...
    uint8_t *value;
    uint16_t value_len;
} ble_gateway_response_t;

static errcode_t ble_gateway_send_value_notification(uint16_t handle,
                                                     const uint8_t *data,
                                                     uint16_t len,
                                                     const char *name);
...
static errcode_t ble_gateway_send_response(uint8_t server_id,
                                           uint16_t conn_id,
                                           const ble_gateway_response_t *response_data)
{
...
static void ble_gateway_read_request_cb(uint8_t server_id,
                                        uint16_t conn_id,
                                        gatts_req_read_cb_t *request,
                                        errcode_t status)
{
    (void)status;
    osal_printk("%s read request received, handle=0x%04x\r\n", BLE_GATEWAY_SERVER_LOG, request->handle);
    if (!request->need_rsp) {
        return;
    }

    if (request->handle != g_data_handle) {
        ble_gateway_response_t response = {request->request_id, GATT_STATUS_INVALID_HANDLE, NULL, 0};
...
static void ble_gateway_handle_cccd_write(uint8_t server_id, uint16_t conn_id, gatts_req_write_cb_t *request)
{
    ...
    if (request->need_rsp) {
        ble_gateway_response_t response = {request->request_id, response_status, NULL, 0};
        (void)ble_gateway_send_response(server_id, conn_id, &response);
    }
                                         ...
                                         errcode_t status)
{
    ...
    if (request->handle == g_notify_cccd_handle) {
        ble_gateway_handle_cccd_write(server_id, conn_id, request);
        return;
    }
    if (request->handle != g_data_handle) {
...
static errcode_t ble_gateway_add_data_characteristic(void)
{
    bt_uuid_t data_uuid = {0};
    gatts_add_chara_info_t characteristic = {0};
    gatts_add_character_result_t data_result = {0};

    ble_gateway_uuid16(BLE_GATEWAY_DATA_UUID, &data_uuid);
...
static errcode_t ble_gateway_add_notify_characteristic(void)
{
    ...
    ble_gateway_uuid16(BLE_GATEWAY_NOTIFY_UUID, &notify_uuid);
    ...
    ble_gateway_uuid16(BLE_GATEWAY_CCCD_UUID, &cccd_uuid);
...
static errcode_t ble_gateway_add_gatt_service(void)
{
    bt_uuid_t app_uuid = {0};
    bt_uuid_t service_uuid = {0};

    ble_gateway_uuid16(BLE_GATEWAY_SERVICE_UUID, &app_uuid);
    errcode_t ret = gatts_register_server(&app_uuid, &g_server_id);
    if (ret != ERRCODE_BT_SUCCESS) {
        return ret;
    }
    ble_gateway_uuid16(BLE_GATEWAY_SERVICE_UUID, &service_uuid);
    ret = gatts_add_service_sync(g_server_id, &service_uuid, true, &g_service_handle);
    if (ret != ERRCODE_BT_SUCCESS) {
        return ret;
    }
    ret = ble_gateway_add_data_characteristic();
    if (ret != ERRCODE_BT_SUCCESS) {
        return ret;
    }
    ret = ble_gateway_add_notify_characteristic();
    if (ret != ERRCODE_BT_SUCCESS) {
        return ret;
    }

    osal_printk("%s service ready: service=0x%04x data=0x%04x notify=0x%04x notify_cccd=0x%04x\r\n",
                BLE_GATEWAY_SERVER_LOG, g_service_handle, g_data_handle, g_notify_handle, g_notify_cccd_handle);
    return gatts_start_service(g_server_id, g_service_handle);
}

static void ble_gateway_service_start_cb(uint8_t server_id, uint16_t handle, errcode_t status)
{
    if (server_id != g_server_id || handle != g_service_handle) {
        return;
    }
    if (status != ERRCODE_BT_SUCCESS) {
        osal_printk("%s service start failed: 0x%x\r\n", BLE_GATEWAY_SERVER_LOG, status);
        return;
    }
...
static errcode_t ble_gateway_configure_and_start(void)
{
    ...
    ret = ble_gateway_add_gatt_service();
    osal_printk("%s init %s, ret=0x%x\r\n", BLE_GATEWAY_SERVER_LOG, ret == ERRCODE_BT_SUCCESS ? "ok" : "failed", ret);
    return ret;
}
...
static errcode_t ble_gateway_register_callbacks(void)
{
    ...
    gatt_callbacks.start_service_cb = ble_gateway_service_start_cb;
    gatt_callbacks.read_request_cb = ble_gateway_read_request_cb;
    gatt_callbacks.write_request_cb = ble_gateway_write_request_cb;
...
errcode_t ble_gateway_server_send_notification(const uint8_t *data, uint16_t len)
{
    ...
    return ble_gateway_send_value_notification(g_notify_handle, data, len, "notification");
}
...
errcode_t ble_gateway_server_init(void)
{
    ...
    errcode_t ret = ble_gateway_register_callbacks();
    if (ret != ERRCODE_BT_SUCCESS) {
        osal_printk("%s callback registration failed: 0x%x\r\n", BLE_GATEWAY_SERVER_LOG, ret);
        return ret;
    }
    return ble_gateway_configure_and_start();
}

...
#define BLE_GATEWAY_ADV_INTERVAL 0x30
#define BLE_GATEWAY_ADV_CHANNEL_MAP 0x07
#define BLE_GATEWAY_ADV_TYPE_CONN_UNDIR 0x00
#define BLE_GATEWAY_ADV_FILTER_ALLOW_ALL 0x00
#define BLE_GATEWAY_ADV_DURATION_FOREVER 0
...
#define BLE_GATEWAY_STATE_RETAINED 0x01
...
{
    g_ble_gateway_default_state = is_default;
}

static uint16_t ble_gateway_build_adv_data(uint8_t *data, uint16_t capacity)
{
    uint16_t index = 0;
    uint16_t name_len = (uint16_t)(sizeof(BLE_GATEWAY_NAME) - 1);
    ...
    data[index++] = (uint8_t)(BLE_GATEWAY_SERVICE_UUID & 0xFF);
    data[index++] = (uint8_t)(BLE_GATEWAY_SERVICE_UUID >> BLE_UUID_HIGH_BYTE_SHIFT);

    data[index++] = (uint8_t)(name_len + 1);
    data[index++] = BLE_AD_TYPE_COMPLETE_NAME;
    if (memcpy_s(&data[index], capacity - index, BLE_GATEWAY_NAME, name_len) != EOK) {
        return 0;
    }
    ...
    data[index++] = (uint8_t)(BLE_GATEWAY_SERVICE_UUID & 0xFF);
    data[index++] = (uint8_t)(BLE_GATEWAY_SERVICE_UUID >> BLE_UUID_HIGH_BYTE_SHIFT);
    data[index++] = g_ble_gateway_default_state ? BLE_GATEWAY_STATE_DEFAULT : BLE_GATEWAY_STATE_RETAINED;
    return index;
}
...
errcode_t ble_gateway_server_start_adv(void)
{
    ...
    uint16_t adv_len = ble_gateway_build_adv_data(adv_data, sizeof(adv_data));
    ...
    ret = gap_ble_set_adv_data(BLE_GATEWAY_ADV_ID, &config);
    if (ret != ERRCODE_BT_SUCCESS) {
        return ret;
    }

    param.min_interval = BLE_GATEWAY_ADV_INTERVAL;
    param.max_interval = BLE_GATEWAY_ADV_INTERVAL;
    param.duration = BLE_GATEWAY_ADV_DURATION_FOREVER;
    param.adv_type = BLE_GATEWAY_ADV_TYPE_CONN_UNDIR;
    param.channel_map = BLE_GATEWAY_ADV_CHANNEL_MAP;
    param.adv_filter_policy = BLE_GATEWAY_ADV_FILTER_ALLOW_ALL;
    param.peer_addr.type = 0;
    (void)memset_s(param.peer_addr.addr, sizeof(param.peer_addr.addr), 0, sizeof(param.peer_addr.addr));

    ret = gap_ble_set_adv_param(BLE_GATEWAY_ADV_ID, &param);
    if (ret != ERRCODE_BT_SUCCESS) {
        return ret;
    }
    return gap_ble_start_adv(BLE_GATEWAY_ADV_ID);
}

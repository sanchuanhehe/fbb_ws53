...
{
    osal_printk("mtu change server_id: %d, conn_id: %d, mtu_size: %d, status: 0x%x\n", server_id, conn_id,
        mtu_size, status);
}

static void ble_wifi_cfg_server_conn_param_update_cbk(uint16_t conn_id, errcode_t status,
    const gap_ble_conn_param_update_t *param)
{
    osal_printk("conn param update conn_id: %d, status: 0x%x\n", conn_id, status);
    if (param != NULL) {
...
static errcode_t ble_wifi_cfg_server_register_callbacks(void)
{
    ...
    if (ret != ERRCODE_BT_SUCCESS) {
        osal_printk("[wifi_cfg_server] reg gap cbk failed, ret=0x%x\r\n", ret);
        return ERRCODE_BT_FAIL;
    }
    ...
    if (ret != ERRCODE_BT_SUCCESS) {
        osal_printk("[wifi_cfg_server] reg service cbk failed, ret=0x%x\r\n", ret);
        return ERRCODE_BT_FAIL;
    }
    return ret;
}
...
errcode_t wifi_cfg_server_service(void)
{
    ...
    ret = gatts_start_service(BLE_WIFI_CFG_SERVER_ID, g_service_hdl);
    osal_printk("[wifi_cfg_server] ble uuid add service out, ret=0x%x\r\n", ret);
    return ret;
}
...
errcode_t ble_wifi_cfg_server_init(void)
{
    ...
    if (ret != ERRCODE_BT_SUCCESS) {
        osal_printk("[wifi_cfg_server] enable BLE failed, ret=0x%x\r\n", ret);
        return ret;
    }
    ...
    if (ret != ERRCODE_BT_SUCCESS) {
        osal_printk("[wifi_cfg_server] init fail, ret=0x%x.\r\n", ret);
    } else {
...
errcode_t ble_wifi_cfg_server_send_report_by_uuid(const uint8_t *data, uint32_t len)
{
    ...
    if (param.value == NULL) {
        osal_printk("[wifi_cfg_server][ERROR] send report alloc failed\r\n");
        return ERRCODE_BT_FAIL;
    }
    if (memcpy_s(param.value, param.value_len, data, len) != EOK) {
        osal_printk("[wifi_cfg_server][ERROR] send report copy failed\r\n");
        osal_vfree(param.value);
        return ERRCODE_BT_FAIL;
    }
    errcode_t ret = gatts_notify_indicate_by_uuid(BLE_WIFI_CFG_SERVER_ID, conn_id, &param);
    osal_vfree(param.value);
    return ret;
}
...
errcode_t ble_wifi_cfg_server_send_report_by_handle(uint16_t attr_handle, const uint8_t *data, uint8_t len)
{
    ...
    errcode_t ret = gatts_notify_indicate(BLE_WIFI_CFG_SERVER_ID, conn_id, &param);
    osal_vfree(param.value);
    return ret;
}

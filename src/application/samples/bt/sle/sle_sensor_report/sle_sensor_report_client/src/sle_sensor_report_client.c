...
static sle_dev_manager_callbacks_t g_sle_sensor_dev_mgr_cbk = {0};
 ...
 * @endif
 */
static void sle_sensor_sle_enable_cbk(uint8_t status)
{
...
static errcode_t sle_sensor_report_seek_cbk_register(void)
{
    ...
    g_sle_sensor_dev_mgr_cbk.sle_enable_cb = sle_sensor_sle_enable_cbk;
    ...
    errcode_t ret = sle_dev_manager_register_callbacks(&g_sle_sensor_dev_mgr_cbk);
    if (ret != ERRCODE_SLE_SUCCESS) {
        osal_printk("%s register device manager callbacks fail: 0x%x\r\n", SENSOR_CLIENT_LOG, ret);
        return ret;
    }

...
本案例使用两块 WS53 开发板演示基于连接态 RSSI 的粗粒度测距。Server 负责广播并接受连接；Client 精确匹配广播名称，连接后每秒读取一次 RSSI，依次经过 7 点中值滤波、指数移动平均（EMA）和对数距离路径损耗模型，输出估算距离及 `near`、`middle`、`far` 分区。Client 还支持 GPIO13 长按触发的 100 cm 一点校准，并将结果保存到 NV。
...
校准硬件使用 WS53 的 GPIO13 和 GPIO5；是否为板载按键/灯取决于具体 WS53 板型，否则需要外接：
...
fbb config --target ws53_liteos_app set CONFIG_SAMPLE_SUPPORT_SLE_SAMPLE=y
fbb config --target ws53_liteos_app set CONFIG_SAMPLE_SUPPORT_SLE_RSSI_RANGING_SERVER_SAMPLE=y
fbb build ws53_liteos_app --clean

fbb config --target ws53_liteos_app set CONFIG_ENABLE_BT_SAMPLE=y
fbb config --target ws53_liteos_app set CONFIG_SAMPLE_SUPPORT_SLE_SAMPLE=y
fbb config --target ws53_liteos_app set CONFIG_SAMPLE_SUPPORT_SLE_RSSI_RANGING_CLIENT_SAMPLE=y
fbb build ws53_liteos_app --clean
```

 ...
 * @endif
 */
static void sle_rssi_enable_cb(uint8_t status)
{
...
{
    errcode_t ret;
    sle_dev_manager_callbacks_t device_manager_callbacks = {0};
    sle_announce_seek_callbacks_t callbacks = {0};

    device_manager_callbacks.sle_enable_cb = sle_rssi_enable_cb;
    ret = sle_dev_manager_register_callbacks(&device_manager_callbacks);
    if (ret != ERRCODE_SLE_SUCCESS) {
        return ret;
    }

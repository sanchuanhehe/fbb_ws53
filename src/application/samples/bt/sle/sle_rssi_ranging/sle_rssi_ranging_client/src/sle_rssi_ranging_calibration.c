...
/*
 * GPIO and task parameters are mapped to the WS53 target used by this sample.
 ...
 * @endif
 */
static inline void sle_rssi_led_delay(uint8_t cycle_count)
{
    uint8_t i;
...
{
    (void)uapi_gpio_set_val(SLE_RSSI_CAL_LED_PIN, GPIO_LEVEL_HIGH);
    sle_rssi_led_delay(SK6805_ONE_HIGH_DELAY);
    (void)uapi_gpio_set_val(SLE_RSSI_CAL_LED_PIN, GPIO_LEVEL_LOW);
...
errcode_t sle_rssi_calibration_init(void)
{
     ...
     */
    (void)uapi_pin_set_mode(SLE_RSSI_CAL_LED_PIN, HAL_PIO_FUNC_GPIO);

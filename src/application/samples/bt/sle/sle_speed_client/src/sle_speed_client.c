...
static sle_dev_manager_callbacks_t   g_dev_mgr_cbk = {0};
...
void sle_sample_sle_enable_cbk(uint8_t status)
{
    if (status == 0) {
        ...
        if (ret != 0) {
            printf("[speed client] ssapc_register_client failed, errcode: %d\r\n", ret);
        return;
        }
    }
}
...
{
    g_dev_mgr_cbk.sle_enable_cb = sle_sample_sle_enable_cbk;
...
void sle_sample_pair_complete_cbk(uint16_t conn_id, const sle_addr_t *addr, errcode_t status)
{
    ...
    osal_printk("[speed client] status: %d\r\n", status);
    if (status == 0) {
...
void sle_client_init(ssapc_notification_callback notification_cb, ssapc_indication_callback indication_cb)
{
    ...
    sle_dev_manager_register_callbacks(&g_dev_mgr_cbk);

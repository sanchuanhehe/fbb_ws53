...
errcode_t sle_speed_server_adv_init(void)
{
    ...
    osal_printk("sle_uuid_server_adv_init start announce.\r\n");
    errno_t ret;
    ret = sle_start_announce(SLE_ADV_HANDLE_DEFAULT);
    if (ret != ERRCODE_SLE_SUCCESS) {
        osal_printk("sle_uuid_server_adv_init,sle_start_announce fail :0x%x\r\n", ret);
        return ret;
    }
    return ERRCODE_SLE_SUCCESS;
}

...
errcode_t sle_ssaps_set_info(void)
{
    ssap_exchange_info_t info = {0};
    info.mtu_size = DEFAULT_SLE_SPEED_MTU_SIZE;
    info.version = 1;
    errcode_t ret = ssaps_set_info(g_server_id, &info);
    osal_printk("[speed server] set info.\r\n");
    return ret;
}

static errcode_t sle_speed_server_add(void)
{
    errcode_t ret;
    ...
    ret = sle_ssaps_set_info();
    if (ret != 0) {
        osal_printk("[speed server] set mtu failed, ret:0x%x\r\n", ret);
        return ERRCODE_SLE_FAIL;
    }
...
{
    unused(status);
    osal_printk("[ssap server] update state changed conn_id:%d, interval = %02x\n", conn_id, param->interval);
#ifdef CONFIG_LARGE_THROUGHPUT_SERVER
    if (g_ssap_handle == NULL) {
        osal_kthread_lock();
        g_ssap_handle = osal_kthread_create((osal_kthread_handler)send_data_thread_function,
            0, "SsapSampleTask", SPEED_DEFAULT_KTHREAD_SIZE);
        if (g_ssap_handle != NULL) {
            osal_kthread_set_priority(g_ssap_handle, SPEED_DEFAULT_KTHREAD_PROI + 1);
            osal_kfree(g_ssap_handle);
        }
        osal_kthread_unlock();
        osal_printk("\r\nkthread success\r\n");
    }
...
{
    osal_printk("[ssap server] conn_id:%d, rssi = %c, status = 0x%x\n", conn_id, rssi, status);
}
...
static void sle_speed_entry(void)
{
    ...
    osal_kthread_lock();
    task_handle = osal_kthread_create((osal_kthread_handler)sle_speed_server_init, 0,
                                      "speed", SPEED_DEFAULT_KTHREAD_SIZE);
    if (task_handle != NULL) {

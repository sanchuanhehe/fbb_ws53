...
本案例使用两块 WS53 开发板演示基于 RSSI 的链路自适应：
...
fbb config --target ws53_liteos_app set CONFIG_SAMPLE_SUPPORT_SLE_PHY_MCS_SWITCH_SERVER_SAMPLE=y
...
```shell
fbb config --target ws53_liteos_app set CONFIG_SAMPLE_SUPPORT_SLE_PHY_MCS_SWITCH_CLIENT_SAMPLE=y
fbb build ws53_liteos_app --clean
```

...
本案例使用两块 WS53 开发板演示 SLE 连接建立后的参数更新。Server 发起更新，Client 输出收到的请求，双方输出最终生效参数。
...
```shell
fbb config --target ws53_liteos_app set CONFIG_SAMPLE_SUPPORT_SLE_CONN_PARAM_TUNING_SERVER_SAMPLE=y
fbb config --target ws53_liteos_app set CONFIG_SLE_CONN_PARAM_PROFILE_BALANCED=y
...
```shell
fbb config --target ws53_liteos_app set CONFIG_SAMPLE_SUPPORT_SLE_CONN_PARAM_TUNING_CLIENT_SAMPLE=y
fbb build ws53_liteos_app --clean
```

## 运行与验收

1. 将 Server 和 Client 固件分别烧录到两块 WS53 开发板。

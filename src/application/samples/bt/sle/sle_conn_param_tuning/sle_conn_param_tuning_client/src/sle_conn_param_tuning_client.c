...
static sle_dev_manager_callbacks_t g_sle_conn_param_dev_mgr_cbk = {0};
 ...
 * @endif
 */
static void sle_conn_param_enable_cb(uint8_t status)
{
...
static errcode_t sle_conn_param_register_seek_callbacks(void)
{
    ...
    g_sle_conn_param_dev_mgr_cbk.sle_enable_cb = sle_conn_param_enable_cb;
    ...
    ret = sle_dev_manager_register_callbacks(&g_sle_conn_param_dev_mgr_cbk);
    if (ret != ERRCODE_SLE_SUCCESS) {
        return ret;
    }

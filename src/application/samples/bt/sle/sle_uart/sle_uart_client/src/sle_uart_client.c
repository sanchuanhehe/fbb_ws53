...
#define SLE_UART_COMPLETE_LOCAL_NAME_TYPE 0x0B
...
static sle_dev_manager_callbacks_t g_dev_manager_cbk = {0};
...
static bool sle_uart_client_has_server_name(const uint8_t *data, uint8_t data_len)
{
    static const uint8_t server_name[] = SLE_UART_SERVER_NAME;
    uint16_t index = 0;
    ...
    while (index < data_len) {
        ...
        if ((data[index + 1U] == SLE_UART_COMPLETE_LOCAL_NAME_TYPE) &&
 ...
 * @endif
 */
static void sle_uart_client_sle_enable_cbk(uint8_t status)
{
...
static void sle_uart_client_seek_result_info_cbk(sle_seek_result_info_t *seek_result_data)
{
    ...
    if (sle_uart_client_has_server_name(seek_result_data->data, seek_result_data->data_length)) {
...
static void sle_uart_client_seek_cbk_register(void)
{
    g_dev_manager_cbk.sle_enable_cb = sle_uart_client_sle_enable_cbk;
    (void)sle_dev_manager_register_callbacks(&g_dev_manager_cbk);

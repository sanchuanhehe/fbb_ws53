...
OAL_STATIC osal_s32 hmac_ccpriv_set_mfg_mode(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    g_mfg_mode = mfg_mode;
    if (g_mfg_mode == 1) {
        (osal_void)hmac_ccpriv_alg_cfg_etc(hmac_vap, (const osal_s8 *)"tpc_mode 0");
        (osal_void)hmac_ccpriv_alg_cfg_etc(hmac_vap, (const osal_s8 *)"aggr_enable 0");
        (osal_void)hmac_ccpriv_alg_cfg_etc(hmac_vap, (const osal_s8 *)"edca_opt_en_sta 0");
    } else {
        (osal_void)hmac_ccpriv_alg_cfg_etc(hmac_vap, (const osal_s8 *)"tpc_mode 1");
        (osal_void)hmac_ccpriv_alg_cfg_etc(hmac_vap, (const osal_s8 *)"aggr_enable 1");
        (osal_void)hmac_ccpriv_alg_cfg_etc(hmac_vap, (const osal_s8 *)"edca_opt_en_sta 1");
    }
...
OAL_STATIC osal_void hmac_mfg_print_output_param(const mfg_param_stru *mfg_param)
{
    osal_u8 index;
...
OAL_STATIC osal_s32 hmac_config_efuse_operate_with_echo(hmac_vap_stru *hmac_vap, efuse_operate_stru *param)
{
    ...
    unref_param(hmac_vap);
    ret = hal_config_efuse_operate(param);
    if (ret != OAL_SUCC) {
        wifi_printf_always("ERROR\n");
    } else {
        wifi_printf_always("OK\n");
...
OAL_STATIC osal_s32 hmac_ccpriv_set_efuse_rssi_offset(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    ret = hal_set_efuse_rssi_offset(rssi_param, RSSI_OFFSET_SIZE);
    if (ret != OAL_SUCC) {
        wifi_printf_always("ERROR\r\n");
        return ret;
    }
...
OAL_STATIC osal_s32 hmac_ccpriv_efuse_write_power_info(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    ret = hal_efuse_write_power_info(power_info, WLAN_CFG_ONE_POWER_PARAM_NUM);
    if (ret != OAL_SUCC) {
        wifi_printf_always("ERROR\r\n");
        return ret;
    }
...
OAL_STATIC osal_s32 hmac_ccpriv_efuse_read_power_info(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    unref_param(hmac_vap);
    unref_param(param);
    ret = hal_read_efuse_read_power_info();
    if (ret != OAL_SUCC) {
        wifi_printf_always("ERROR\n");
        return ret;
    }
...
OAL_STATIC osal_s32 hmac_ccpriv_efuse_write_temp(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    ret = hal_efuse_write_temp(gears);
    if (ret != OAL_SUCC) {
        wifi_printf_always("ERROR\n");
        return ret;
    }
...
OAL_STATIC osal_s32 hmac_ccpriv_efuse_get_remain(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    ret = hal_config_efuse_operate(&efuse_info);
    if (ret != OAL_SUCC) {
        wifi_printf_always("ERROR\n");
        return ret;
    }
...
static osal_s32 hmac_ccpriv_set_jtag_enable(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    efuse_info.val[0] = ((osal_u8)jtag_enable << 1) + (osal_u8)jtag_enable;
...
static osal_s32 hmac_ccpriv_set_ssi_spi_mask(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    (osal_void)memset_s(&efuse_info, sizeof(efuse_info), 0, sizeof(efuse_info));
    efuse_info.val[0] = ((osal_u8)ssi_spi_mask << 1) + (osal_u8)ssi_spi_mask;
    efuse_info.op = EFUSE_OP_WRITE;

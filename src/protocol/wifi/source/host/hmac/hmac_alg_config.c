...
OAL_STATIC OAL_CONST wal_ioctl_alg_intrf_mode_stru g_alg_intrf_mode_cfg_map_etc[] = {
    ...
    {"descend_11b_switch",      MAC_ALG_INTRF_MODE_DESCEND_11B_SWITCH, {0, 0, 0}},
...
OSAL_STATIC osal_s32 hmac_ccpriv_alg_intrf_mode_mixed(hmac_vap_stru *hmac_vap, const osal_s8 *param)
{
    ...
    struct {
        ...
        {BIT1, MAC_ALG_INTRF_MODE_DESCEND_11B_SWITCH},   // descend 11b 映射到bit1
    };
    frw_msg msg_info = {0};
...
OSAL_STATIC osal_s32 hmac_config_ar_set_tx_rate(hmac_vap_stru *hmac_vap_tmp, frw_msg *msg)
{
    ...
    /* 管理帧/VIP数据帧统一走单播管理帧配置通道 */
    if (tx_rate_param->cfg_frame_type < ALG_SET_FIX_PARAMS_TYPE_DATA) {
        frame = ALG_FRAME_TYPE_UCAST_MGMT;
    }
    ...
    hmac_set_al_tx_protocol(tx_rate_param->protocol);
    hmac_set_al_tx_mcs(tx_rate_param->rate_index);
#endif
...
/* 入参有效性由调用者保证 */
OSAL_STATIC osal_void hmac_query_alg_tx_rate_stats_detail(frw_msg *msg, osal_s32 ret, osal_u8 value)
{
...
OSAL_STATIC osal_void hmac_query_alg_rx_rate_stats_detail(const alg_rx_rate_all_stats_stru *stats)
{
    wifi_printf("\r\n");

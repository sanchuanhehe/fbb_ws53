...
#define COEX_SWITCH_LEVEL_HIGH 0
#define COEX_SWITCH_LEVEL_LOW 1
#define T1_TIMER_MIN 60
#define T1_TIMER_MAX 220
#define T2_TIMER_MIN 0
#define T2_TIMER_MAX 20
#define TIMER_GAP 10
...
typedef osal_u8 btcoex_wire_mode_uint8;

typedef enum {
    WIFI_BT,
    WIFI_BSLE_BT,
}btcoex_mode;
typedef osal_u8 btcoex_mode_uint8;

typedef enum {
    HMAC_EXTCOEX_T1_TIMER = 0,
    HMAC_EXTCOEX_T2_TIMER = 1,
    HMAC_EXTCOEX_T3_TIMER = 2,
    HMAC_EXTCOEX_T4_TIMER = 3,
    HMAC_EXTCOEX_TIMER_BUTT
}wlan_extcoex_timer;
typedef osal_u8 wlan_extcoex_timer_uint8;
...
typedef union {
    ...
    struct {
        osal_u8 cfg_extcoex_en  : 1;         /* [0]    */
        osal_u8 cfg_coex_mode   : 2;         /* [2..1] */
        osal_u8 cfg_share_ant   : 1;         /* [3]    */
        osal_u8 cfg_coex_switch : 1;         /* [4]    */
        osal_u8 cfg_premmpt_en  : 2;         /* [6..5] */
        osal_u8 cfg_rsv         : 1;         /* [7]    */
    } bits;
} u_coex_custom;
...
typedef union {
    ...
    /* Define the struct bits */
    struct {
        osal_u32 cfg_t1_timer  : 16;         /* [15..0]  */
        osal_u32 cfg_t2_timer  : 16;         /* [31..16] */
    } bits;
} u_coex_custom_timer;
...
/*****************************************************************************
  函数声明
*****************************************************************************/
osal_void hal_set_g_extcoex_timer(osal_u8 index, osal_u8 val);
osal_void hal_set_extcoex_timer(osal_u16 t1_timer, osal_u16 t2_timer, osal_u16 t4_timer);
...
osal_void hal_extcoex_common_init(osal_u8 preempt_enable);
osal_void hal_extcoex_mode_init(osal_u8 wire_mode, osal_u8 coex_mode, osal_bool share_ant);
osal_void hal_dev_sync_extcoex_status_to_dev(osal_u8 extcoex_status);
osal_void hal_extcoex_init(osal_void);
...
osal_void hal_set_extcoex_gpio(osal_void);

#ifdef _PRE_BSLE_GATEWAY
osal_void hal_bsle_set_priority(osal_void);
#endif
osal_bool hal_btcoex_get_bt_run(osal_void);
osal_void hal_set_extcoex_preemp(osal_u16 value);
osal_void hal_set_extcoex_off(osal_void);
osal_void hal_set_extcoex_switch_config(osal_u16 value);

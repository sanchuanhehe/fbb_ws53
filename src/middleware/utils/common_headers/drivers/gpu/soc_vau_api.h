...
/**
 * @ingroup vau_api
 * @brief blur Interfaces CNcomment:模糊接口 CNend
 *
 * @par Description:
 * path area and rect area blur can be implemented through the blur interface.\n
 * CNcomment:路径和矩形区域的高斯模糊可通过模糊接口实现。CNend
 *
 * @param handle       [IN] type #td_handle Task handle CNcomment:任务句柄 CNend
 * @param surface_list [IN] type #src blur bg surface and mask surface parameters CNcomment:要模糊的背景层和蒙版层 CNend
 *
 * @retval #0          Success
 * @retval #Other      Failure
 *
 ...
 * @li soc_vau_type.h
 */
td_s32 uapi_vau_blur(td_handle handle, ext_vau_surface_list *surface_list);

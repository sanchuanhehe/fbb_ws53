...
#endif
#if defined(BUILD_APPLICATION_STANDARD) || defined(TEST_SUITE) || defined(UNIT_TEST)
#if CORE == APPS
...
#define print_init()
#if defined(BUILD_SELITEOS)
    #define PRINT(fmt, arg...) print_str("SELITEOS|" fmt, ##arg)
...
#elif CORE == APPS
    #define PRINT(fmt, arg...) osal_printk("APP|" fmt, ##arg)
#elif CORE == SECURITY
...
#elif CORE == SENSOR
    #define PRINT(fmt, arg...) print_str("sensor|" fmt, ##arg)
#else
    #define PRINT(fmt, arg...)
#endif

#elif defined(BOOT_ROM_DFR_PRINT) && defined(YES) && BOOT_ROM_DFR_PRINT == YES
void sw_debug_uart_init(uint32_t baud_rate);
void print_str(const char *str, ...);
#define PRINT(fmt, arg...) print_str("ROM|" fmt, ##arg)
...
#define print_init()
#define PRINT(fmt, arg...) osal_printk(fmt, ##arg)
#else
#define print_init()
#define PRINT(fmt, arg...)

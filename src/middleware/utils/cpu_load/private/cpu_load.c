...
#ifndef CONFIG_BT_SUPPORT_REAL_ROM
...
static const section_head_t g_section_information __attribute__((section(".build_info"))) = {
    ...
    {{SECTION_TYPE_INIT, &g_system_stack_begin, &g_system_stack_end, NULL, RESEVED_VALUE},
     ...
     {SECTION_TYPE_RELOAD, NULL, (void *)(uintptr_t)BT_VECTORS_LENGTH,
      (void *)(uintptr_t)BT_VECTORS_LOAD_ORIGIN, RESEVED_VALUE},
#if (CHIP_SW39 == 1 || CHIP_SW21 == 1)
#ifndef CONFIG_BT_SUPPORT_XIP_RUN
...
static const section_head_t g_section_information __attribute__((section(".build_info"))) = {
    ...
    0,
    {{SECTION_TYPE_INIT, &g_system_stack_begin, &g_system_stack_end, NULL, RESEVED_VALUE},
     {SECTION_TYPE_RELOAD, NULL, (void *)(uintptr_t)SENSOR_VECTORS_LENGTH,
      (void *)(uintptr_t)SENSOR_VECTORS_LOAD_ORIGIN, RESEVED_VALUE},
     {SECTION_TYPE_RELOAD, &__text_begin__, &__text_end__, &__text_load__, RESEVED_VALUE},
     {SECTION_TYPE_RELOAD, &__data_begin__, &__data_end__, &__data_load__, RESEVED_VALUE},
     {SECTION_TYPE_CLEAR, &__bss_begin__, &__bss_end__, NULL, RESEVED_VALUE}}
};
...
#if (CHIP_SW39 == 1 || CHIP_SW21 == 1)
#define DSP_REG_BASE_ADDR         0xe8000000
#define DSP_REG_BASE_MCU_ADDR     0x58000000
/* addr in audio sub tranfer to addr in mcu sub by subtracting an offset */
#define DSP_ADDR_MCU_OFFSET       (DSP_REG_BASE_ADDR - DSP_REG_BASE_MCU_ADDR)

#if (CONFIG_FPGA_SUPPORT_VERSION == FPGA_VERSION_85P) /* Default branch */
#define DRV_DSP0_DTCM_SIZE        0xC0000 /* 768K */
#define DRV_DSP0_ITCM_SIZE        0xC0000
#define DRV_DSP0_DTCM_ADDR        (DSP_REG_BASE_ADDR + 0x600000) /* Global addr */
#define DRV_DSP0_ITCM_ADDR        (DSP_REG_BASE_ADDR + 0x700000) /* Global addr */
#define DRV_DSP0_INNER_DTCM_ADDR  (DSP_REG_BASE_ADDR + 0x900000) /* Local addr */
#define DRV_DSP0_INNER_ITCM_ADDR  (DSP_REG_BASE_ADDR + 0xA00000) /* Local addr */

#define DRV_DSP1_DTCM_SIZE        0x40000 /* 256K */
#define DRV_DSP1_ITCM_SIZE        0x20000 /* 128K */
#define DRV_DSP1_DTCM_ADDR        (DSP_REG_BASE_ADDR + 0xE00000) /* Global addr */
#define DRV_DSP1_ITCM_ADDR        (DSP_REG_BASE_ADDR + 0xF00000) /* Global addr */
#define DRV_DSP1_INNER_DTCM_ADDR  (DSP_REG_BASE_ADDR + 0xC00000) /* Local addr */
#define DRV_DSP1_INNER_ITCM_ADDR  (DSP_REG_BASE_ADDR + 0xC40000) /* Local addr */

#else /* FPGA_VERSION_95P */
#define DRV_DSP0_DTCM_SIZE        0xC0000 /* 768K */
#define DRV_DSP0_ITCM_SIZE        0xC0000
#define DRV_DSP0_DTCM_ADDR        (DSP_REG_BASE_ADDR + 0x800000) /* Global addr */
#define DRV_DSP0_ITCM_ADDR        (DSP_REG_BASE_ADDR + 0x900000) /* Global addr */
#define DRV_DSP0_INNER_DTCM_ADDR  (DSP_REG_BASE_ADDR + 0xA00000) /* Local addr */
#define DRV_DSP0_INNER_ITCM_ADDR  (DSP_REG_BASE_ADDR + 0xB00000) /* Local addr */
...
#endif /* end of macro CONFIG_FPGA_SUPPORT_VERSION */

#define DRV_DSP0_INNER_DTCM_MAX   (DRV_DSP0_INNER_DTCM_ADDR + DRV_DSP0_DTCM_SIZE) /* Local addr max */
#define DRV_DSP0_INNER_ITCM_MAX   (DRV_DSP0_INNER_ITCM_ADDR + DRV_DSP0_ITCM_SIZE) /* Local addr max */
#define DRV_DSP1_INNER_DTCM_MAX   (DRV_DSP1_INNER_DTCM_ADDR + DRV_DSP1_DTCM_SIZE) /* Local addr max */
#define DRV_DSP1_INNER_ITCM_MAX   (DRV_DSP1_INNER_ITCM_ADDR + DRV_DSP1_ITCM_SIZE) /* Local addr max */

/* Load addr offset */
#define DRV_DSP0_INNER_DTCM_MCU_OFFSET (DRV_DSP0_INNER_DTCM_ADDR - (DRV_DSP0_DTCM_ADDR - DSP_ADDR_MCU_OFFSET))
#define DRV_DSP0_INNER_ITCM_MCU_OFFSET (DRV_DSP0_INNER_ITCM_ADDR - (DRV_DSP0_ITCM_ADDR - DSP_ADDR_MCU_OFFSET))
#define DRV_DSP1_INNER_DTCM_MCU_OFFSET (DRV_DSP1_INNER_DTCM_ADDR - (DRV_DSP1_DTCM_ADDR - DSP_ADDR_MCU_OFFSET))
#define DRV_DSP1_INNER_ITCM_MCU_OFFSET (DRV_DSP1_INNER_ITCM_ADDR - (DRV_DSP1_ITCM_ADDR - DSP_ADDR_MCU_OFFSET))
...
static uint32_t cpu_load_caculate_addr(uint32_t section_run_begin, core_images_e image)
{
    struct {
        ...
        {DRV_DSP0_INNER_DTCM_MCU_OFFSET, DRV_DSP0_INNER_DTCM_ADDR, DRV_DSP0_INNER_DTCM_MAX}, /* dsp0 dtcm */
        {DRV_DSP0_INNER_ITCM_MCU_OFFSET, DRV_DSP0_INNER_ITCM_ADDR, DRV_DSP0_INNER_ITCM_MAX}, /* dsp0 itcm */
        {DRV_DSP1_INNER_DTCM_MCU_OFFSET, DRV_DSP1_INNER_DTCM_ADDR, DRV_DSP1_INNER_DTCM_MAX}, /* dsp1 dtcm */
        {DRV_DSP1_INNER_ITCM_MCU_OFFSET, DRV_DSP1_INNER_ITCM_ADDR, DRV_DSP1_INNER_ITCM_MAX}, /* dsp1 itcm */
    };
...
#if CHIP_SW21 == 1
        return (section_run_begin + GNSS_RAM_ORIGIN_APP_MAPPING);
#endif
    }
    if (image == CORE_IMAGES_EXTERN1) {
#if CHIP_SW21 == 1
        return (section_run_begin + SENSOR_RAM_ORIGIN_APP_MAPPING);
#endif
    }
    if (image == CORE_IMAGES_PROTOCOL) {
        for (uint32_t i = 0; i < array_size(dsp_section_map); i++) {
            if (section_run_begin >= dsp_section_map[i].start && section_run_begin < dsp_section_map[i].end) {
                PRINT("audio map 0x%x by subtracting 0x%x\r\n", section_run_begin, dsp_section_map[i].offset);
                return (section_run_begin - dsp_section_map[i].offset); /* Audio sub TCM */
            }
        }
        if (section_run_begin >= DSP_REG_BASE_ADDR) {
            PRINT("audio map 0x%x by subtracting 0x%x\r\n", section_run_begin, DSP_ADDR_MCU_OFFSET);
            return (section_run_begin - DSP_ADDR_MCU_OFFSET); /* Audio sub non-TCM addr */
        }
    }
...
#endif
#if CHIP_SW21
        return 0x21000000; //!< GNSS REMAP_ADDR_OFFSET = 0x84000000 / 4
#endif
    }
    return 0;
}
...
uint32_t* cpu_load_translate_into_mcu_xip_addr(uint32_t *addr, core_images_e image)
{
    ...
    } else if ((image == CORE_IMAGES_EXTERN0) && (addr != NULL)) {
        return (uint32_t*)((uint32_t)addr);
    } else if ((image == CORE_IMAGES_EXTERN1) && (addr != NULL)) { // for 1a21 sensor
        return (uint32_t*)((uint32_t)addr);
    } else {
#ifdef BUILD_APPLICATION_STANDARD
        oml_pf_log_print2(LOG_BCORE_PLT_LPM_DSP_POWER, LOG_NUM_LPM_DSP_POWER, LOG_LEVEL_INFO, \
...
void cpu_load_image(const section_head_t *load_info, core_images_e image)
{
    ...
    for (uint32_t i = 0; i < head->section_num; i++) {
        ...
        PRINT("copy sections num = %x, type = %x, load_addr = %x, run_addr_gegin = %x, section_size = %x\r\n",
              i, type, load_addr, run_addr_begin, section_size);
#endif

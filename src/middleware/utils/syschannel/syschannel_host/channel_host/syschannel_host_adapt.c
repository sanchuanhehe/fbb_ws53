...
#define HOST_HCC_TASK_PRIORITY 70
#else
...
static syschannel_stat_stru g_syschann_host_stat = {0};
...
syschannel_stat_stru *syschannel_host_get_trx_stat(osal_void)
{
    return &g_syschann_host_stat;
}
...
static osal_void syschannel_host_sync_stat_clean(osal_void)
{
    memset_s(&g_syschann_host_stat, sizeof(g_syschann_host_stat), 0, sizeof(g_syschann_host_stat));
    return;
}

static osal_u32 syschannel_host_sync_stat_num(osal_void)
{
    osal_u8 ret;
    osal_u8 buf[SYSCHANNEL_MSG_STAT_STRU_LEN];
    ...
    /* 发消息将数据同步到device */
    osal_printk("syschann_host:pkt tx s:%d,f:%d,rx s:%d, rx alloc f:%d\r\n"
        "app msg tx s:%d,f:%d,rx s:%d;mng msg tx s:%d,f:%d,rx s:%d,rx msg alloc f:%d\r\n",
        g_syschann_host_stat.pkt_tx, g_syschann_host_stat.pkt_tx_fail, g_syschann_host_stat.pkt_rx,
        g_syschann_host_stat.pkt_rx_alloc_fail,
        g_syschann_host_stat.app_msg_tx, g_syschann_host_stat.app_msg_tx_fail, g_syschann_host_stat.app_msg_rx,
        g_syschann_host_stat.mng_msg_tx, g_syschann_host_stat.mng_msg_tx_fail,
        g_syschann_host_stat.mng_msg_rx, g_syschann_host_stat.msg_rx_alloc_fail);
 
    buffer = buf;
    buffer[0] = SYSCHANNEL_HSOT_SYNC_TRX_STAT;
    buffer++;
    memcpy_s(buffer, sizeof(g_syschann_host_stat), &g_syschann_host_stat, sizeof(g_syschann_host_stat));
    ret = syschannel_tx_msg_adapt(syschannel_handler->hcc_id, buf, SYSCHANNEL_MSG_STAT_STRU_LEN,
        SYSCHANNEL_SERVICE_TYPE_MSG, SYSCHANNEL_SUB_TYPE_MSG_MNT);
    if (ret != OSAL_OK) {
        g_syschann_host_stat.mng_msg_tx_fail++;
        osal_printk("syschannel_host_sync_stat_num tx failed\r\n");
    }
    g_syschann_host_stat.mng_msg_tx++;
    return ret;
}
...
static osal_u32 syschannel_host_rx_msg_mnt(osal_u8 *buf, osal_u32 len)
{
    ...
    switch (msg_mnt_type) {
        ...
        case SYSCHANNEL_HOST_SYNC_IP_ADDR:
            ...
            break;
        case SYSCHANNEL_HSOT_SYNC_TRX_STAT:
            syschannel_host_sync_stat_num();
            break;
        case SYSCHANNEL_HSOT_SYNC_TRX_STAT_CLEAN:
            syschannel_host_sync_stat_clean();
            break;
        default:
            break;
    }
    ...
    osal_u8 *buf, osal_u32 len, osal_u8 *user_param)
{
    ...
    switch (stype) {
        case SYSCHANNEL_SUB_TYPE_MSG_MNT:
            g_syschann_host_stat.mng_msg_rx++;
        ...
        case SYSCHANNEL_SUB_TYPE_MSG_APP:
            g_syschann_host_stat.app_msg_rx++;
...
STATIC osal_u32 syschannel_host_alloc_msg_buf(hcc_queue_type queue_id, osal_u32 len, osal_u8 **buf,
    osal_u8 **user_param)
{
    unref_param(queue_id);
    ...
    *user_param = OSAL_NULL;
    if (*buf == OSAL_NULL) {
        g_syschann_host_stat.msg_rx_alloc_fail++;
        return OSAL_NOK;
    }
    return OSAL_OK;
}
    ...
    hcc_adapt_priv_rx_process rx_process)
{
    ...
    if (service_type == SYSCHANNEL_SERVICE_TYPE_PKT) {
        ...
        adapt->alloc = syschannel_host_alloc_msg_buf;
...
{
    hcc_message_register(hcc_id, 0, D2H_MSG_BSP_READY, syschannel_wait_bsp_ready, (td_u8 *)&g_bsp_ready_wait);
    hcc_message_register(hcc_id, 0, D2H_MSG_HEARTBEAT, syschannel_d2h_headbeat, NULL);
    hcc_message_register(hcc_id, 0, D2H_MSG_SUSPEND_ACK, syschannel_wait_suspend_ack, (td_u8 *)&g_suspend_ack_wait);
}
...
osal_u32 syschannel_host_init(osal_void)
{
    ...
    if (ret != OSAL_OK) {
        osal_wait_destroy(&g_bsp_ready_wait);
        osal_wait_destroy(&g_suspend_ack_wait);
...
osal_u32 hcc_test_cmd_ext(const osal_char *buf, size_t count)
{
    ...
    if ((cmd_str = osal_strstr(buf, SYSCHANNEL_SUSPEND)) != OSAL_NULL) {
        ...
        td_char *str_param = cmd_str + osal_strlen(SYSCHANNEL_RESUME);
...
td_u32 hcc_test_cmd_show_ext(osal_char *buf)
{
    return snprintf_s(buf, SYSCHANNEL_SHOW_BUF_LEN, SYSCHANNEL_SHOW_BUF_LEN, "%d\r\n", g_syschannel_status);
}
#ifdef __cplusplus
#if __cplusplus
}

...
osal_s32 netdev_register(osal_void)
{
    ...
    /* 初始化netif名称 */
    if (strncpy_s(netif->name, IFNAMSIZ, "wlan", IFNAMSIZ - 2) != EOK) { /* 2 size减2 */
...
osal_u32 syschannel_host_sync_ip_addr(osal_u8 *buf, osal_u32 len)
{
    ...
    netif_set_addr(g_lwip_netif, ip_addr_stru->ip_addr, ip_addr_stru->mask_addr, ip_addr_stru->gw_addr);

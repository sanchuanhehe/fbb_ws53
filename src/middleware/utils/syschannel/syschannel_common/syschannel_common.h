...
/* 大小端转换 */
#if defined(SYSCHANNEL_BIG_ENDIAN) && defined(SYSCHANNEL_ENDIAN) \
    && (SYSCHANNEL_BIG_ENDIAN == SYSCHANNEL_ENDIAN) /* BIG_ENDIAN */
...
typedef enum {
    ...
    SYSCHANNEL_HSOT_SYNC_TRX_STAT = 5,      /* Syschannel同步host统计的数据给device */
    SYSCHANNEL_HSOT_SYNC_TRX_STAT_CLEAN = 6,
    SYSCHANNEL_MSG_MAX = 0xff,
} syschannel_msg_mnt;

#define SYSCHANNEL_MSG_STAT_STRU_LEN (SYSCHANNEL_MSG_TYPE_LEN + sizeof(syschannel_stat_stru))

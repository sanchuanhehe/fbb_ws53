...
#undef THIS_FILE_ID
#define THIS_FILE_ID DIAG_FILE_ID_HCC_COMM_HCC_ADAPT_C
...
hcc_unc_struc *hcc_alloc_unc_buf(hcc_handler *hcc)
{
    ...
    if (unc_pool == TD_NULL) {
        unc_buf = (hcc_unc_struc *)osal_kmalloc(sizeof(hcc_unc_struc), OSAL_GFP_KERNEL);
    } else {
...
td_void hcc_free_unc_buf(hcc_handler *hcc, hcc_unc_struc *unc_buf)
{
    ...
    hcc_dfx_unc_free_cnt_increase(hcc);
}
    ...
    hcc_handler *hcc, hcc_queue_type queue_id, td_u32 buf_len, td_u8 **buf, td_u8 **user_param)
{
    ...
    if (serv_info->adapt == TD_NULL) {
        hcc_printf_err_log("hcc alloc srv-%d null\r\n", serv_type);
        return EXT_ERR_FAILURE;
    }

    if (serv_info->adapt->alloc == TD_NULL) {
        hcc_printf_err_log("hcc alloc priv srv- %d\r\n", serv_type);
        return EXT_ERR_FAILURE;
    }

    hcc_dfx_service_alloc_cb_cnt_increase(hcc, serv_type);
...
td_void hcc_adapt_free_priv_buf(hcc_handler *hcc, hcc_queue_type queue_id, td_u8 *buf, td_u8 *user_param)
{
    ...
    if (serv_info == TD_NULL) {
        hcc_printf_err_log("hcc free srv err, q-%d, srv-%d\r\n", queue_id, serv_type);
        return;
    }

    if (serv_info->adapt == TD_NULL) {
        hcc_printf_err_log("hcc free adapt srv- %d null\r\n", serv_type);
        return;
    }

    if (serv_info->adapt->free != TD_NULL) {
        serv_info->adapt->free(queue_id, buf, user_param);
        hcc_dfx_service_free_cnt_increase(hcc, serv_type);
        return;
    }
    hcc_printf_err_log("hcc adapt free-%d null\r\n", serv_type);
}
...
ext_errno hcc_bus_rx_handler(hcc_bus *bus, td_u8 queue_id, hcc_data_queue *assembled_head)
{
    ...
    while (unc_buf != TD_NULL) {
        ...
        if (!hcc_check_header_vaild(hcc, (hcc_header *)unc_buf->buf)) {
            hcc_printf_err_log("data invalid index:%u, len:%u\r\n", rx_index, unc_buf->length);

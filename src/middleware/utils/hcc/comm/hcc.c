...
STATIC td_void hcc_service_resource_deinit(hcc_handler *hcc)
{
    osal_spin_lock_destroy(&hcc->hcc_serv.service_lock);
    osal_spin_lock_destroy(&hcc->hcc_serv_del.service_lock);
}

STATIC ext_errno hcc_service_resource_init(hcc_handler *hcc)
{
...
STATIC ext_errno hcc_set_unc_pool(hcc_data_queue *unc_pool, td_u32 unc_pool_size)
{
    ...
    if (hcc_list_head_init(unc_pool) != EXT_ERR_SUCCESS) {
        return EXT_ERR_FAILURE;
    }

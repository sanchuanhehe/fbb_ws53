...
#undef THIS_FILE_ID
#define THIS_FILE_ID DIAG_FILE_ID_HCC_COMM_HCC_LIST_C
...
STATIC struct osal_list_head *hcc_list_peek_tail(TD_CONST struct osal_list_head *head)
{
...
{
    struct osal_list_head *node = hcc_queue->data_queue.next;
    if (node == &hcc_queue->data_queue) {
        return TD_NULL;
    } else {
        hcc_list_unlink(node);
        hcc_queue->qlen--;
        return osal_list_entry(node, hcc_unc_struc, list);
    }
}

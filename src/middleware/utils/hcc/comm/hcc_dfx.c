...
ext_errno hcc_dfx_init(hcc_handler *hcc)
{
    ...
    if (tmp_hcc_dfx->service_stat == TD_NULL) {
        osal_kfree(tmp_hcc_dfx->queue_stat[HCC_DIR_TX]);
...
td_void hcc_dfx_deinit(hcc_handler *hcc)
{
#ifdef CONFIG_HCC_SUPPORT_DFX
    hcc_dfx *tmp_hcc_dfx = TD_NULL;
    if (hcc == TD_NULL) {
        return;
    }
    tmp_hcc_dfx = hcc->dfx_handle;
    if (tmp_hcc_dfx == TD_NULL) {
        return;
    }
    if (tmp_hcc_dfx->queue_stat[HCC_DIR_TX] != TD_NULL) {
        osal_kfree(tmp_hcc_dfx->queue_stat[HCC_DIR_TX]);
        tmp_hcc_dfx->queue_stat[HCC_DIR_TX] = TD_NULL;
    }
    if (tmp_hcc_dfx->queue_stat[HCC_DIR_RX] != TD_NULL) {
        osal_kfree(tmp_hcc_dfx->queue_stat[HCC_DIR_RX]);
        tmp_hcc_dfx->queue_stat[HCC_DIR_RX] = TD_NULL;
    }
#ifdef CONFIG_HCC_SUPPORT_DFX_SRV
    if (tmp_hcc_dfx->service_stat != TD_NULL) {
        osal_kfree(tmp_hcc_dfx->service_stat);
        tmp_hcc_dfx->service_stat = TD_NULL;
    }
#endif
    osal_spin_lock_destroy(&tmp_hcc_dfx->hcc_dfx_lock);
    osal_kfree(tmp_hcc_dfx);
    hcc->dfx_handle = TD_NULL;
#else
    uapi_unused(hcc);
#endif
    return;
...
td_void hcc_dfx_queue_info_print(td_u8 chl, hcc_queue_dir dir, hcc_queue_type q_id)
{
    ...
    if (hcc_dfx_stat == TD_NULL || q_id >= hcc_dfx_stat->que_max_cnt || dir >= HCC_DIR_COUNT) {
        return;
    }

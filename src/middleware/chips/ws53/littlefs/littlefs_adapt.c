...
#define LFS_FLASH_START 0x400000
#define LFS_FLASH_4K_MASK 0xFFF
#define LFS_FLASH_4K 0x1000
#define LFS_RAM_BASE 0x20000000
    ...
    lfs_size_t size)
{
    ...
    lfs_debug_print_info("littlefs_adapt_read enter blocksize%u block %u off %u size %u\r\n", c->block_size, block,
        off, size);
    ret = uapi_sfc_reg_read(c->block_size * (g_lfs_block_info.start_block + block) + off, (uint8_t *)buffer, size);
    lfs_debug_print_info("littlefs_adapt_read ret %d\r\n", ret);
    if (ret != ERRCODE_SUCC) {
        return (int)ret;
    }
    return LFS_ERR_OK;
...
{
    errcode_t ret = ERRCODE_FAIL;
    lfs_debug_print_info("littlefs_adapt_write enter blocksize%u block %u off %u size %u\r\n", c->block_size, block,
        off, size);
...
static int littlefs_adapt_erase(const struct lfs_config *c, lfs_block_t block)
{
    ...
    lfs_debug_print_info("littlefs_adapt_erase enter %u %u\r\n", start_sector, c->block_size);
...
void fs_adapt_mount(void)
{
    ...
    if (ret < LFS_ERR_OK) {
        ...
        if (ret < LFS_ERR_OK) {
            lfs_debug_print_error("lfs_format failed, ret = 0x%x\r\n", ret);
        }
        ret = lfs_mount(&g_lfs, &cfg);
        lfs_debug_print_info("lfs_mount 2, ret = 0x%x\r\n", ret);
        if (ret < LFS_ERR_OK) {
            lfs_debug_print_error("lfs_mount failed, ret = 0x%x\r\n", ret);
        }
    }
}
...
int fs_adapt_mkdir(const char *path)
{
    ...
    if (ret == LFS_ERR_EXIST) {
        lfs_debug_print_info("dir exists, ret = 0x%x, dir = %s\r\n", ret, path);
        return LFS_ERR_OK;
    }
    if  (ret < 0) {
        lfs_debug_print_error("lfs_mkdir failed, ret = 0x%x, dir = %s\r\n", ret, path);
        return -1;
    }
    return ret;
}
...
int fs_adapt_open(const char* path, int oflag)
{
    ...
    real_flag |= LFS_O_CREAT;
    ret = lfs_file_open(&g_lfs, fp, name, real_flag);
    if (ret < LFS_ERR_OK) {
        lfs_debug_print_error("lfs_file_open failed, ret = 0x%x, name = %s\r\n", ret, name);
        free(fp);
        return -1;
    }
    g_lfs_open_file_cnt++;
...
int fs_adapt_close(int fd)
{
    ...
    if  (ret < 0) {
        lfs_debug_print_error("lfs_file_close failed, ret = 0x%x\r\n", ret);
...
int fs_adapt_read(int fd, char *buf, unsigned int len)
{
    ...
    if  (ret < 0) {
        lfs_debug_print_error("lfs_file_read failed, ret = 0x%x\r\n", ret);
        return -1;
    }
...
int fs_adapt_write(int fd, const char *buf, unsigned int len)
{
    ...
    if  (ret < 0) {
        lfs_debug_print_error("lfs_file_write failed, ret = 0x%x\r\n", ret);
        return -1;
    }
...
int fs_adapt_delete(const char *path)
{
    ...
    if  (ret < 0) {
        lfs_debug_print_error("lfs_remove failed, ret = 0x%x\r\n", ret);
        return -1;
    }
...
int fs_adapt_stat(const char *path, unsigned int *file_size)
{
    ...
    if (ret < 0) {
        lfs_debug_print_error("lfs_stat failed, ret = 0x%x\r\n", ret);
        return -1;
    }
    *file_size = info.size;
...
int fs_adapt_seek(int fd, int offset, unsigned int whence)
{
    ...
    if  (ret < 0) {
        lfs_debug_print_error("lfs_file_seek failed, ret = 0x%x\r\n", ret);
        return -1;
    }
...
int fs_adapt_sync(int fd)
{
    ...
    if  (ret < 0) {
        lfs_debug_print_error("fs_adapt_sync failed, ret = 0x%x\r\n", ret);
        return -1;
    }
    return ret;
}

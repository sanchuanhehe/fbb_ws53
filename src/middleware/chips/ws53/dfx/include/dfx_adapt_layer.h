 ...
 */
static inline int32_t transmit_msg_write(uint32_t msg_id, uint8_t *msg, uint16_t msg_len, bool wait)
{
    return (int32_t)dfx_msg_write(msg_id, msg, msg_len, wait);
}
...
static inline errcode_t dfx_timer_init(dfx_timer *timer, dfx_timer_handler handler, uintptr_t data, uint32_t ms)
{
    ...
    timer->data = data;
    timer->interval = ms;
    return (osal_timer_init(timer) == OSAL_SUCCESS) ? ERRCODE_SUCC : ERRCODE_FAIL;
}

...
static void zdiag_adapt_sdt_om_log_parse(uint8_t *sdt_buf, uint16_t sdt_buf_size, diag_msg_para *para)
{
...
static void zdiag_adapt_sdt_om_status_parse(uint8_t *sdt_buf, uint16_t sdt_buf_size, diag_msg_para *para)
{
...
static void zdiag_adapt_sdt_om_ota_parse(uint8_t *sdt_buf, uint16_t sdt_buf_size, diag_msg_para *para)
{
...
static void zdiag_adapt_sdt_om_last_parse(uint8_t *sdt_buf, uint16_t sdt_buf_size, diag_msg_para *para)
{
...
static void zdiag_adapt_sdt_dscr_cb_parse(uint8_t *sdt_buf, uint16_t sdt_buf_size, diag_msg_para *para)
{
    om_dscr_cb_data_stru_t *om_dscr = (om_dscr_cb_data_stru_t*)sdt_buf;

    para->module_id = om_dscr->module_id;
    para->msg_id = om_dscr->msg_id;
    para->buf = om_dscr->data;
    para->buf_size = om_dscr->data_len;
    para->level = DIAG_LOG_LEVEL_WARN;
    unused(sdt_buf_size);
}
...
{
    zdiag_adapt_sdt_msg *msg = (zdiag_adapt_sdt_msg*)data;
    ...
    if (g_recv_msg_step == SDT_MSG_STEP_HEAD) {
        ...
        if (frame_len - recv_len > 0) {
            ret = (uint32_t)memcpy_s(g_recv_buf + recv_len, frame_len - recv_len, msg->msg, msg->len);
            if (ret != EOK) {
                dfx_free(0, msg->msg);
                return ret;
            }
    ...
    if (data_len >= sizeof(om_msg_header_stru_t)) {
        ...
        if ((om_msg_header->frame_start == OM_FRAME_DELIMITER) &&
            ...
            if (om_msg_header->func_type ==  OM_MSG_TYPE_LOG) {
                ...
                report = true;
            } else if (om_msg_header->func_type == OM_MSG_TYPE_DSCR) {
                zdiag_adapt_sdt_dscr_cb_parse(data, (uint16_t)data_len, &para);
                report = true;
            }
            pack = false;
...
void zdiag_adapt_sdt_msg_proc(uint8_t *buf1, uint32_t len1, uint8_t *buf2, uint32_t len2)
{
    ...
    if (len1 > 0) {
        ret = (uint32_t)memcpy_s(buf + pos, len1, buf1, len1);
        if (ret != EOK) {
            return;
        }
        pos += len1;
    ...
    if (len2 > 0) {
        ret = (uint32_t)memcpy_s(buf + pos, len2, buf2, len2);
        if (ret != EOK) {
            return;
        }
    }
    ...
    if (pack) {
        msg.msg = buf;
        msg.len = len;
        ret = dfx_msg_write((uint32_t)DFX_MSG_ID_SDT_MSG, (uint8_t *)&msg,
            sizeof(zdiag_adapt_sdt_msg), false);
        if (ret != ERRCODE_SUCC) {
            dfx_free(0, buf);
        }
    } else {
...
void diag_auto_log_report_enable(void)
{
    ...
    /* 全局使能 */
    zdiag_set_enable(0, (uint8_t)diag_adapt_get_default_dst());  // 0 默认关闭HSO通道
    ...
    for (uint32_t i = 0; i < sizeof(level) / sizeof(uint32_t); i++) {
        zdiag_set_level_enable((uint8_t)level[i], 1);
    }
}
#endif

#if (CORE_NUMS > 1)
static void diag_notify_log_level(uint8_t level)
{
    ipc_msg_info_t head = {0};
    head.dst_core = CORES_BT_CORE;
    head.priority = 1;
    head.msg_id = IPC_MSG_LOG_LEVEL;
    head.buf_addr = (uint8_t *)&level;
    head.buf_len = (size_t)sizeof(uint8_t);
    if (uapi_ipc_send_msg_async(&head) != ERRCODE_SUCC) {
        dfx_log_err("diag_highest_level_proc, notify ccore log level ipc err.\r\n");
    }
}
/* hso工具log级别下发 */
void diag_highest_level_proc(uint8_t level)
{
    ...
    diag_notify_log_level((uint8_t)core_level);
}

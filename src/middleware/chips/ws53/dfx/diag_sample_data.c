                                           ...
                                           diag_option_t *option)
{
    ...
    if (cmd_id == DIAG_CMD_ID_WLAN_SAMPLE) {
        diag_wlan_sample_cmd *wlan_cmd = (diag_wlan_sample_cmd *)cmd_param;
        g_sample_running.report_mode = wlan_cmd->report_mode;
    }
...
static errcode_t diag_cmd_report_data(const uint8_t *buf, uint32_t len)
{
    ...
    uapi_diag_report_packet((uint16_t)DIAG_CMD_ID_SAMPLE_DATA, NULL, (uint8_t *)reply, (uint16_t)reply_size, true);
    dfx_free(0, reply);
    return ERRCODE_SUCC;
}

errcode_t diag_cmd_report_finish(void)
{
...
errcode_t diag_report_wlan_sample_data(const uint8_t *buf, uint32_t len, uint32_t msg_id)
{
    ...
    if (g_sample_running.report_mode == DIAG_WLAN_REPORT_MESSAGE) {

...
#else
    uint32_t irq = (uint32_t)osal_irq_lock();
    tmp_ptr = malloc(size);
...
errcode_t mfg_flash_write_info(const uint8_t *ram_data, uint32_t flash_offset, uint32_t size, bool do_erase)
{
    ...
    if (ret != ERRCODE_SUCC) {
        ...
        ret = ERRCODE_FACTORY_FLASH_WRITE_ERROR;
        goto write_failed;
    }
    osal_irq_restore(lock);
    /* 回读比较 */
    ret = uapi_sfc_reg_read(flash_offset, cmp_data, size);
    if (ret != ERRCODE_SUCC) {
        ret = ERRCODE_FACTORY_FLASH_READ_ERROR;
        goto write_failed;
    }
    ret = (errcode_t)memcmp(cmp_data, ram_data, size);
    if (ret != EOK) {
        ret = ERRCODE_FACTORY_MEMCMP_VERIFY_ERROR;
        goto write_failed;
    }
write_failed:
    mfg_free(cmp_data);
...
uint32_t mfg_get_region_config(mfg_region_config_t *region_cfg)
{
    ...
    ret |= uapi_partition_get_info(PARTITION_CCPU_IMAGE, &(region_cfg->app_info));
    ret |= uapi_partition_get_info(PARTITION_FOTA_DATA, &(region_cfg->mfg_info));
    return ret;
}

#ifdef WS53_PRODUCT_NONE
...
#define MAC_LEN 6
#endif
#define MAC_ADDR_NUM 2  /* MAC地址数量: MAC、SLE MAC */

#ifdef CONFIG_MIDDLEWARE_SUPPORT_NV
static errcode_t mfg_get_nv_mac_from_backup(void)
{
    errcode_t nv_ret = ERRCODE_SUCC;
    uint16_t mac_nv_key_arr[MAC_ADDR_NUM] = {NV_ID_SYSTEM_FACTORY_SLE_MAC, NV_ID_SYSTEM_FACTORY_MAC};
    /* 如果主NV区sle mac或者wifi mac是无效数据，且
    * 备NV区sle mac或者wifi mac是有效数据，则反备份sle mac或者wifi mac到主分区
    */
    /* 获取主备NV中的MAC、SLE MAC */
    for (uint8_t i = 0; i < MAC_ADDR_NUM; ++i) {
        uint8_t mac_nv[MAC_LEN] = {0};
        uint8_t mac_nv_backup[MAC_LEN] = {0};
        nv_key_attr_t nv_attr = {0};

        uint16_t length = 0;
        nv_ret = uapi_nv_read_with_attr(mac_nv_key_arr[i], MAC_LEN, &length, mac_nv, &nv_attr);
        if (nv_ret != ERRCODE_SUCC) {
            osal_printk("nv mac read fail: id: 0x%x, ret:0x%x\r\n", mac_nv_key_arr[i], nv_ret);
            if (nv_ret == ERRCODE_NV_KEY_NOT_FOUND) {
                // 若是没有该项ID，则跳过
                continue;
            }
            return nv_ret;
        ...
        length = 0;
        nv_ret = kv_read_factory(mac_nv_key_arr[i], MAC_LEN, &length, mac_nv_backup);
        if (nv_ret != ERRCODE_SUCC) {
            osal_printk("backup nv mac read fail: id: 0x%x, ret:0x%x\r\n", mac_nv_key_arr[i], nv_ret);
            if (nv_ret == ERRCODE_NV_KEY_NOT_FOUND) {
                // 若是没有该项ID，则跳过
                continue;
            }
        ...
        if (mac_addr_is_zero(mac_nv) && !mac_addr_is_zero(mac_nv_backup)) {
            /* 备NV区mac、slemac反备份到主NV区 */
            nv_ret = uapi_nv_write_with_attr(mac_nv_key_arr[i], mac_nv_backup, MAC_LEN, &nv_attr, NULL);
            if (nv_ret != ERRCODE_SUCC) {
                osal_printk("nv mac write fail: id: 0x%x, ret:0x%x\r\n", mac_nv_key_arr[i], nv_ret);
                return nv_ret;
            }
        }
    }
...
errcode_t mfg_factory_mode_switch(mfg_region_config_t img_info, uint32_t switch_mode, mfg_factory_config_t *config)
{
    ...
    if (switch_mode == 0x0) { /* switch to normal mode */
        if (config->factory_mode != 0x0) {
            ...
            /* nv backup */
#ifdef CONFIG_MIDDLEWARE_SUPPORT_NV
            errcode_t nv_ret = mfg_get_nv_mac_from_backup();
            if (nv_ret != ERRCODE_SUCC) {
                return nv_ret;
            }
            ...
            nv_ret = uapi_nv_backup((const nv_backup_mode_t *)&backup_flag);
            if (nv_ret != ERRCODE_SUCC) {
                return nv_ret;
            }
#endif

...
static upg_image_partition_ids_map_t g_img_partition_map[] = {
    ...
    {APPLICATION_IMAGE_ID, PARTITION_CCPU_IMAGE},
};
...
uint32_t upg_get_ids_map(upg_image_partition_ids_map_t **map)
{
    *map = g_img_partition_map;
    return (sizeof(g_img_partition_map) / sizeof(g_img_partition_map[0]));
}
...
static uint32_t g_ws53_support_upg_id[] = {
    FLASH_BOOT_IMAGE_ID,
    APPLICATION_IMAGE_ID,
    UPG_IMAGE_ID_NV,
};
static upg_image_collections_t g_ws53_upg_collect = { g_ws53_support_upg_id, sizeof(g_ws53_support_upg_id) /
    sizeof(g_ws53_support_upg_id[0])};
/* 获取当前程序支持的升级镜像 */
upg_image_collections_t *uapi_upg_get_image_id_collection(void)
{
    return &g_ws53_upg_collect;
}
...
errcode_t upg_flash_read(const uint32_t flash_offset, const uint32_t size, uint8_t *ram_data)
{
    ...
    ret = uapi_sfc_dma_read(flash_offset, (uint8_t *)ram_data, size);
    if (ret != ERRCODE_SUCC) {
#elif defined (CONFIG_DRIVER_SUPPORT_SFC)
...
#if defined (CONFIG_SFC_SUPPORT_DMA)
    ret = uapi_sfc_dma_write(flash_offset, (uint8_t *)ram_data, size);
    if (ret != ERRCODE_SUCC) {
#elif defined (CONFIG_DRIVER_SUPPORT_SFC)
...
STATIC errcode_t verify_public_rootkey_etc(root_public_key *rootkey_buff)
{
    ...
    /* check hash */
    if (memcmp(hash_from_otp, hash_result, (uint32_t)SHA_256_LENGTH) != 0) {
...
#if (UPG_CFG_VERIFICATION_SUPPORT == YES)
static errcode_t upg_get_efuse_addr(uint32_t image_id, uint32_t *start_addr, uint32_t *ver_size)
{
    switch (image_id) {
        case FLASH_BOOT_IMAGE_ID:
            *start_addr = (EFUSE_FLASHBOOT_ROLLBACK_VERSION / EFUSE_BYTE_TO_BIT);
            *ver_size = EFUSE_FLASHBOOT_VERSION_SIZE;
            break;
        case APPLICATION_IMAGE_ID:
            *start_addr = (EFUSE_APP_ROLLBACK_VERSION / EFUSE_BYTE_TO_BIT);
            *ver_size = EFUSE_APP_VERSION_SIZE;
            break;
        default:
            return ERRCODE_FAIL;
    }
...
errcode_t upg_get_board_rollback_version(uint32_t image_id, uint32_t *anti_rollback_ver)
{
    ...
    uint32_t ver_size;
    errcode_t ret = upg_get_efuse_addr(image_id, &efuse_offset, &ver_size);
    if (ret != ERRCODE_SUCC) {
        return ret;
    }
    return (errcode_t)uapi_efuse_read_buffer((uint8_t *)anti_rollback_ver, efuse_offset, (uint16_t)ver_size);
}
...
errcode_t upg_set_board_rollback_version(uint32_t image_id, uint32_t *anti_rollback_ver)
{
    ...
    ret = uapi_efuse_write_buffer(efuse_offset, (uint8_t *)&version, (uint16_t)ver_size);
...
static void ws53_upg_putc(const char c)
{
#ifndef WS53_PRODUCT_NONE
    unused(c);
#else
...
errcode_t ws53_upg_init(void)
{
    ...
    upg_func.serial_putc = ws53_upg_putc;

...
#if defined(CONFIG_SUPPORT_CRASHINFO_SAVE_TO_FLASH) || defined(CONFIG_SUPPORT_RST_RSN_SAVE_TO_FLASH)
#include "partition.h"
#endif
...
#define REBOOT_WAIT_TIME_MS        3000
...
#if defined(CONFIG_SUPPORT_CRASHINFO_SAVE_TO_FLASH) || defined(CONFIG_SUPPORT_RST_RSN_SAVE_TO_FLASH)
bool crashinfo_flash_addr_get(uint32_t *flash_save_addr, uint32_t *flash_save_size)
{
...
{
    uint32_t flash_save_addr = 0, flash_save_size = 0, flash_save_offset = 0;
    ...
    flash_save_offset += (uint32_t)(sizeof(uint32_t) + sizeof(uint32_t));
    ...
    uint32_t excpts_val = read_custom_csr(CXCPTSC);
    uapi_sfc_reg_write(flash_save_offset, (uint8_t *)(uintptr_t)&excpts_val, sizeof(uint32_t));
    flash_save_offset += (uint32_t)sizeof(uint32_t);
    ...
    mcpu_trace_save(flash_save_offset, flash_save_size - (flash_save_offset - flash_save_addr));
#endif
    uapi_sfc_reg_write(flash_save_addr + (uint32_t)sizeof(uint32_t),
        (uint8_t *)(uintptr_t)&g_crashinfo_flag, sizeof(uint32_t));
}
...
bool crashinfo_status_get(void)
{
    ...
    uapi_sfc_reg_read(flash_save_addr + (uint32_t)sizeof(uint32_t),
        (uint8_t *)(uintptr_t)&save_flag, sizeof(uint32_t));
...
void crashinfo_dump(void)
{
    ...
    uint32_t save_flag = 0;
    uint32_t flash_save_offset = flash_save_addr + (uint32_t)sizeof(uint32_t);
    ...
    uint32_t excpts_val = 0;
    uapi_sfc_reg_read(flash_save_offset, (uint8_t *)(uintptr_t)&excpts_val, sizeof(uint32_t));
    PRINT("cxcptsc = 0x%x\n", excpts_val);
    flash_save_offset += (uint32_t)sizeof(uint32_t);
    ...
    flash_save_offset += STACK_SAVE_SIZE;
#ifdef CONFIG_ACORE_CPU_TRACE
    mcpu_trace_dump(flash_save_offset);
#endif
    PRINT("Dump end\r\n");
}
#endif
...
void do_hard_fault_handler(exc_context_t *exc_buf_addr)
{
    ...
    uint32_t rst_cause = (exc_type == NMI_INTERRUPT) ? REBOOT_BY_ACORE_WDT : REBOOT_BY_ACORE_EXCEPTION;
    reset_cause_set(rst_cause);
#endif
    PRINT("exception:%x\r\n", exc_type);
    ...
    deal_with_exception(exc_buf_addr, exc_type, 0);
}
...
#endif
#ifdef CONFIG_SUPPORT_EXCEPT_REBOOT
    cpu_utils_reset_chip_with_log((cores_t)MASTER_BY_ALL, reset_cause);
#endif
#ifdef CONFIG_SUPPORT_EXCEPT_DISABLE_WDT
    uapi_watchdog_disable();
#endif
#ifdef CONFIG_SUPPORT_EXCEPT_WAITFOREVER
    while (1) {}
#endif
}

static void ccore_wdt_irq_handler(void)
{
#ifdef CONFIG_SUPPORT_RST_RSN_SAVE_TO_FLASH
...
/* c核看门狗中断注册 */
void ccore_wdt_irq_register(void)
{
    osal_irq_request(B_WDT_IRQN, (osal_irq_handler)ccore_wdt_irq_handler, NULL, NULL, NULL);

...
dev_addr_stru g_sle_mac_addr = {0};
...
#if defined(CONFIG_DRIVER_SUPPORT_EFUSE) || defined (CONFIG_MIDDLEWARE_SUPPORT_NV)
uint8_t mac_addr_is_zero(const uint8_t *mac_addr)
{
...
uint32_t get_dev_addr(uint8_t *pc_addr, uint8_t addr_len, uint8_t type)
{
    ...
    if (type == IFTYPE_SLE) {
        if (memcmp(g_sle_mac_addr.ac_addr, zero_mac, WLAN_MAC_ADDR_LEN) != 0) {
            for (tmp = 0; tmp < WLAN_MAC_ADDR_LEN; tmp++) {

...
VENDOR_LED_FILE = 'vendor/Example_WS53_Board/demo/blinky/blinky.c'
VENDOR_BEEP_FILE = 'vendor/Example_WS53_Alt/demo/uart/uart.c'
...
MOCK_ENTRIES = [
    {
        'file_path': './vendor/Example_WS53_Board/build_config.json',
        'build_target': 'ws53_liteos_app',
        'sample_name': 'demo-blinky',
        'platform': 'WS53',
        'build_defs': [],
    },
    {
        'file_path': './vendor/Example_WS53_Board/build_config.json',
        'build_target': 'ws53_liteos_app',
        'sample_name': 'demo-uart',
        'platform': 'WS53',
        'build_defs': ['CONFIG_SAMPLE_BEEP=y'],
    },
    {
        'file_path': './vendor/Example_WS53_Alt/build_config.json',
        'build_target': 'ws53_liteos_app',
        'sample_name': 'demo-uart',
        'platform': 'WS53',
        'build_defs': [],
...
class TestCheckChangesAndGetFolders(unittest.TestCase):
    ...
    def test_only_vendor_changes(self):
        ...
        self.assertIn('Example_WS53_Board+demo+blinky', result)
...
class TestExtractExactMatch(unittest.TestCase):
    ...
    def test_match_found(self):
        ...
        match_list = {'Example_WS53_Board+demo+blinky'}
        result = gate.extract_exact_match(MOCK_ENTRIES, match_list)
        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]['sample_name'], 'demo-blinky')

    def test_multiple_matches(self):
        """多个vendor变更匹配多个条目"""
        match_list = {
            'Example_WS53_Board+demo+blinky',
            'Example_WS53_Board+demo+uart',
        }
        result = gate.extract_exact_match(MOCK_ENTRIES, match_list)
...
class TestArchivePackaging(unittest.TestCase):
    """archives zip packaging."""

    def test_package_archives_zips_logs_and_fwpkg(self):
        with tempfile.TemporaryDirectory() as archive_dir:
            files = {
                'build-a.log': 'log-a',
                'build-b.log': 'log-b',
                'sample-a.fwpkg': 'fwpkg-a',
                'sample-b.fwpkg': 'fwpkg-b',
                'README.txt': 'ignored',
            }
            ...
            with patch.object(gate, 'ARCHIVE_DIRECTORY', archive_dir):
                gate.package_archives()

            log_zip = os.path.join(archive_dir, gate.BUILD_LOG_ZIP_NAME)
            fwpkg_zip = os.path.join(archive_dir, gate.FWPKG_ZIP_NAME)

            self.assertTrue(os.path.exists(log_zip))
            self.assertTrue(os.path.exists(fwpkg_zip))

            with zipfile.ZipFile(log_zip) as zf:
                self.assertEqual(zf.namelist(), ['build-a.log', 'build-b.log'])

            with zipfile.ZipFile(fwpkg_zip) as zf:
                self.assertEqual(zf.namelist(), ['sample-a.fwpkg', 'sample-b.fwpkg'])
...
class TestMainFlow(unittest.TestCase):
                ...
                with patch.object(gate, 'compile_sdk_and_save_log') as mock_compile:
                    with patch.object(gate, 'extract_exact_match') as mock_extract:
                        with patch.object(gate, 'process_build_info_files'):
                            with patch.object(gate, 'package_archives') as mock_package:
                                gate.main()

        mock_compile.assert_called_once_with('ws53_liteos_app')
        mock_extract.assert_not_called()
        mock_package.assert_called_once_with()
        self.assertEqual(gate.global_combined, 'ws53_liteos_app_sdk-full_WS53')
    ...
    def test_both_src_and_vendor_goes_vendor_path(self):
        ...
        with patch.object(gate, 'compare_with_remote_master',
                          return_value={'Example_WS53_Board+demo+blinky'}):
            with patch.object(gate, 'process_build_info_files', return_value=MOCK_ENTRIES):

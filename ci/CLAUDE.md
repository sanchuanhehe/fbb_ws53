...
2. **返回空 set 且 has_src_changes == True** — 仅 src/ 变更 (或 ci_gate.py 自身变更), 直接全量编译整个 SDK:
   ...
   ```
   编译日志输出到 `archives/build-ws53_liteos_app.log`。
...
- 本地测试: `python3 ci/test_ci_gate.py`
- CI 执行:   `python3 ci/ci_gate.py`
...
check_changes_and_get_folders()
    │
    ├── 仅 src 变更  ──→ compile_sdk_and_save_log('ws53_liteos_app')
    │
...
| 构建入口 | `src/build.py` |
| menuconfig | `src/build/config/target_config/ws53/menuconfig/acore/ws53_liteos_app.config` |

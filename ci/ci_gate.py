...
MENUCONFIG_FILE = 'src/build/config/target_config/ws53/menuconfig/acore/ws53_liteos_app.config'
SAMPLE_DIRECTORY = 'src/application/samples'
BUILD_DIRECTORY = 'src'
BUILD_SCRIPT = 'build.py'
OUTPUT_FWPKG_PATH = 'output/ws53/fwpkg/pack_all_core/ws53_liteos_app/ws53_liteos_app_all_in_one.fwpkg'
ARCHIVE_DIRECTORY = 'archives'
BUILD_LOG_ZIP_NAME = 'build_log.zip'
FWPKG_ZIP_NAME = 'ws53_liteos_app_all_in_one.zip'
...
def process_build_info_files() -> List[dict]:
    ...
    if not os.path.isdir(vendor_root):
        _warn(f"vendor directory does not exist: {vendor_root}")
        return result_list
...
def _zip_archived_files(archive_dir: str, extension: str, zip_name: str) -> None:
    """Zip direct children under archives/ matching extension."""
    if not os.path.isdir(archive_dir):
        _warn(f"archive directory does not exist: {archive_dir}")
        return
    ...
    if not matched_files:
        _warn(f"no {extension} files found in {archive_dir}, skip {zip_name}")
        return
    ...
    os.chmod(zip_path, stat.S_IRWXU | stat.S_IRGRP | stat.S_IXGRP | stat.S_IROTH | stat.S_IXOTH)
    _ok(f"archive zip generated: {zip_path}")
